...
> 🔀 **这是 [`LessXi`](https://github.com/LessXi) 的 fork**：上游是 [`WestFox-AwA/dsh-prompt-optimizer`](https://github.com/WestFox-AwA/dsh-prompt-optimizer) 的 `v0.1.1-beta.1`（BSD-3-Clause），已改名为 **`@lessxi/dsh-prompt-optimizer`**（当前 **0.4.31**），以便与上游并行安装、独立迭代。
>
...
本 fork：**[LessXi](https://github.com/LessXi)** · 当前版本 **0.4.31**（插件内 `?` 面板最底部也保留上游署名）
...
- **本地诊断日志默认不写**：平时打字、拦截、切换档位都不会落盘或发请求；只有未捕获的错误与自愈失败会留证。日志只记长度、计数、状态与指纹，**不记你的输入原文**（识别为路径/密钥的片段会被替换）；单个日志超过 2MB 自动轮转，仅保留最近两代，全部保存在本机、不会上传。可在帮助面板内**一键清空**。
