...
> 🔀 **这是 [`LessXi`](https://github.com/LessXi) 的 fork**：上游是 [`WestFox-AwA/dsh-prompt-optimizer`](https://github.com/WestFox-AwA/dsh-prompt-optimizer) 的 `v0.1.1-beta.1`（BSD-3-Clause），已改名为 **`@lessxi/dsh-prompt-optimizer`**（当前 **0.4.34**），以便与上游并行安装、独立迭代。
>
...
> 🌐 **界面语言说明**：插件界面**跟随 DSH 的界面语言**（`zh` / `en`）。DSH 用中文时插件即为中文；DSH 为英文时，控件行、下拉菜单、帮助面板、迷你窗与全部提示都会切换为英文（未收录的条目回落中文原文）。语言只在运行时读取，不写盘、不影响你的对话语言。
> *UI note: the plugin follows the DSH interface language (`zh` / `en`) across controls, menus, help panel, mini panel and every notice. English README is also documentation.*
...
本 fork：**[LessXi](https://github.com/LessXi)** · 当前版本 **0.4.34**（插件内 `?` 面板最底部也保留上游署名）
...
> 输入框左侧的三个控件，从左到右是：**档位**（下拉菜单）、**发送**（下拉菜单）、**会话默认 / 模型名**（下拉菜单），再右边是 **使用帮助（?）**。三个菜单都可用键盘操作（↑ ↓ / Home / End 环绕、Enter 选中、Esc 关闭并回到触发按钮），点 `?` 有同样的简明教程与署名。
...
| 控件 | 取值 | 说明 |
|---|---|---|
| **优化档位** | 关闭 / 普通 / 高级 / 极端 | 关闭＝完全不拦截，恢复原生发送；普通＝只把话说清楚（约 3 秒）；高级＝补上"显然需要"的约束与验收标准（约 20 秒）；极端＝**读取项目真实情况**（只读，不写盘），仅用于把命令里的模糊指代落到实处（约 20 秒） |
...
| 优化很慢 | 高级/极端档需要 20 秒左右（极端档还会只读查证项目）。想快就用**普通档** |
...
| 界面能换英文吗 | 可以。插件**跟随 DSH 的界面语言**：把 DSH 切到 English 即全英文（含控件行、菜单、帮助面板、迷你窗与提示）。插件不单独设语言开关 |
| 优化结果似乎没参考上文 | 「把上面那个函数改一下」这类指代依赖上下文。在**模型菜单**里确认「会话背景」是开启的（默认开启）；关闭后插件不读取任何历史消息 |
...
- 优化请求只发送**你的输入文本**（「重新生成」时另含上一版转达稿与本次修正方向）；**极端档**会额外在**你的项目根目录内**做只读查证（`read` / `glob` / `grep`），只用于确认指代，不写盘、不执行命令、不联网。
- **会话背景（默认开启，可随时关闭）**：开启时会额外把**本会话最近的对话**（最多 6 轮）作为消歧背景发给优化模型，用于补齐「上面那个」「继续」这类指代。提示中写死了边界：只补指代，不得新增需求，与原文冲突以原文为准。关闭后**不读取本会话任何历史**。开关在**模型菜单**内，浮窗元信息行会显示已参考的轮数。
- **本地诊断日志默认不写**：平时打字、拦截、切换档位都不会落盘或发请求；只有未捕获的错误与自愈失败会留证。日志只记长度、计数、状态与指纹，**不含任何输入原文**；另有一个默认关闭的**「诊断取证」**开关（模型菜单底部），开启后才会把输入与结果的前几十字写进本机日志用于排查，可随时关掉（日志中识别为路径/密钥的片段一律替换）；单个日志超过 2MB 自动轮转，仅保留最近两代，全部保存在本机、不会上传。可在帮助面板内**一键清空**。
