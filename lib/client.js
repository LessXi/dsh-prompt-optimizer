...
const MAX_REQUEST_CHARS = 32000;
		...
		const TIERS = [
			...
			{ id: "extreme", label: "极端", hint: "读项目消歧，约 20 秒以上", help: "读项目真实情况，仅用于确认指代（约 20 秒）" },
		];
		const PERMISSIONS = [
			...
			try {
				fetch(API + "/evidence/clear", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" })
					...
					.then((d) => {
						...
						settle("已清空 " + n + " 项", n ? "本地诊断日志已清空（" + n + " 项 · " + kb + " KB）" : "本地诊断日志已清空（本来就没有日志）", 3200);
					})
					.catch((err) => {
						const s = String((err && err.message) || err);
						const stale = s.indexOf("http-404") >= 0 || s.indexOf("http-405") >= 0;
						beacon("logs-clear-failed", { reason: s });
						settle(stale ? "重启后可用" : "清空失败", stale ? "清空日志需要先重启一次 DSH —— 宿主端更新只有在重启后生效" : "清空日志失败：" + s, 3600);
					});
			} catch (e) {
				settle("清空失败", "清空日志失败：" + String((e && e.message) || e), 3200);
			}
		}
...
function beacon(stage, data) {
		...
		function helpPopover() {
			...
			return h("div", {
				...
				h("div", { className: "dpo-help-sec" }, "隐私"),
				...
				h("div", { className: "dpo-pop-foot" },
					h("button", {
						...
						title: "删除本机诊断日志（不影响设置、优化历史与任何会话内容）",
						onClick: clearLogs,
					}, store.clearLogsLabel || "清空日志"),
			...
			return h("div", {
				...
				h("div", { className: "dpo-pop-priv", "data-dpo": "ctx-row" },
					h("button", {
						...
						title: store.useContext !== false ? "点击关闭：关闭后不再读取本会话的任何历史消息" : "点击开启：只读取本会话最近的对话，仅用于消歧",
		...
		function startRun(request, tier, forceError, opts) {
			...
			if (reqText.length > MAX_REQUEST_CHARS) {
				...
				showNotice("输入过长（" + reqText.length + " 字，上限 " + MAX_REQUEST_CHARS + "）· 请精简后重试");
				return null;
			}
		...
		function runPanes() {
			...
			return h("div", { className: "dpo-run", "data-dpo": "run" },
				h("div", { className: "dpo-run-status", "data-dpo": "run-status", "aria-live": "polite" },
					...
					run.contextTurns > 0 && run.status !== "error" ? h("span", { className: "dpo-tok-chip", "data-dpo": "ctx-turns", title: "已把本会话最近 " + run.contextTurns + " 轮对话作为消歧背景提供给优化模型（可在模型菜单里关闭）" }, "背景 " + run.contextTurns + " 轮") : null,
				...
				h("div", { className: "dpo-pane", "data-dpo": "pane-reasoning", "data-kind": "reasoning", "data-live": String(live && !run.reasoning), "aria-busy": String(Boolean(live && !run.reasoning)) },
					h("div", { className: "dpo-pane-title" },
						...
						chips && chips.reasoning
							...
							: (!live && run.status !== "error" ? h("span", { className: "dpo-tok-chip dpo-tok-muted", "data-dpo": "token-reasoning-none", title: "该 provider 未上报思考 token" }, "— tok") : null),
		...
		function overlayFooter() {
			...
			if (store.regenAsk === true) {
				return h("div", { className: "dpo-overlay-actions", "data-dpo": "foot-regen" },
					...
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "regen-cancel", onClick: () => { store.regenAsk = false; emit(); window.setTimeout(() => { try { const b = document.querySelector('[data-dpo="regen"]'); if (b && b.focus) b.focus({ preventScroll: true }); } catch (e) { /* noop */ } }, 80); } }, "取消"),
				);
			}
			if (done && store.permission === "review") {
				return h("div", { className: "dpo-overlay-actions", "data-dpo": "foot-review" },
					...
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "release", title: "按原文直接发出，不优化", onClick: releaseOriginal }, "放行本条"),
					h("button", {
			...
			if (err) {
				return h("div", { className: "dpo-overlay-actions", "data-dpo": "foot-error" },
					...
					h("button", { type: "button", className: "dpo-btn primary", "data-dpo": "release", title: "按原文直接发出，不优化", onClick: releaseOriginal }, "放行本条"),
				);
			}
		...
		function Overlay() {
			...
			return h("div", {
				...
				"data-dpo": "overlay", role: "dialog", tabIndex: -1, "aria-label": "提示词优化浮窗",
				...
				h("div", { className: "dpo-overlay-head", "data-dpo": "drag-handle", onPointerDown, title: "按住拖动（右下角可改大小）" },
					...
					h("span", { className: "dpo-head-hint" }, "↘ 可改大小"),
				),
				...
				h("div", { className: "dpo-overlay-scroll" },
					...
					"发送：" + (PERMISSIONS.find((x) => x.id === store.permission) || {}).label + (store.intercepts.length > 0 ? " · 拦截累计 " + store.intercepts.length : ""),
					(() => {
						...
						return h("button", { type: "button", className: "dpo-jump-trace", "data-dpo": "jump-trace", title: "跳到查证轨迹", onClick: () => {
