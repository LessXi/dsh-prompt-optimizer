window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 弹层 portal 用。DSH 自身的 client 插件即用同一 ModuleLoader 形态取 react-dom
		// （见 @deepseek-ai/dsh-client-ui-renderer/lib/client.js:11 `let react_dom = require("react-dom");`）。
		const ReactDOM = require("react-dom");
		const h = React.createElement;

		const API = "/prompt-optimizer/api";
		const NS = "dsh-prompt-optimizer";
		/* v141：诊断遥测白名单。除下列「故障信号 / 用户可见失败」外，所有 beacon 都门控在
		   debugEvidence 之后（默认关）。动因：keydown-any 在用户打字时每 800ms 发一次 POST，
		   宿主端每次同步 appendFileSync ⇒ 产品里持续的网络请求 + 磁盘写入。
		   门控后仍保留故障可观测性：错误类与槽位自愈类照常上报。
		   注意 "logs-cleared" 故意不在表内 —— 用户主动清空日志后不该再留下任何记录；
		   但清空失败（logs-clear-failed）必须留证。 */
		const ALWAYS_BEACON = new Set([
			"client-error", "client-rejection", "overlay-error", "catalog-error",
			"remount-controls", "remount-give-up", "controls-healed",
			"remount-overlay", "overlay-healed", "overlay-slot-missing",
			"request-too-long", "logs-clear-failed", "run-host-lost",
		]);
		...
		const store = {
			...
			tier: "basic",
			permission: "review",
			// v120：会话背景（最近对话作为消歧背景喂给优化模型）——隐私相关，可随时关闭
			useContext: true,
			/* v140：清空本地诊断日志按钮的瞬时状态（不进 /state，无需持久化） */
			clearLogsLabel: null,
			clearLogsBusy: false,
			/* v141：诊断遥测开关（默认关）。由 /state 下发，详见 ALWAYS_BEACON 注释。 */
			debugEvidence: false,
			/* v154：用户是否手动调过「诊断取证」—— 与 useContextTouched 同构，挡住迟到的 /state 回写 */
			debugEvidenceTouched: false,
			/* v154：设置文档写入是否正常（null=未知 / "ok" / "error"）。失败时在帮助面板给可操作提示。 */
			settingsSync: null,
			modelLabel: "会话默认",
			modelSel: null,
			modelCatalog: null,
		...
		/** 四个弹层的渲染出口：统一 portal 到 document.body。
		 *
		 *  为什么必须 portal：.dpo-pop 若留在 composer 子树里，会被 DSH 的
		 *  .wSkVaW_composerSeat{position:sticky;z-index:7} 关进一个有效层级只有 7 的层叠上下文；
		 *  而浮窗 .dpo-overlay 在 .pI_x6G_overlayLayer{position:absolute;z-index:20} 内（有效层级 20）。
		 *  20 > 7 ⇒ 浮窗永远盖住弹层，此时把 .dpo-pop 的 z-index 提到 1000 也不生效。
		 *  portal 到 body 后，.dpo-pop 自身的 z-index:90（lib/client.js 的 .dpo-pop{position:fixed;z-index:90}）
		 *  直接在根层叠上下文里比较，90 > 20，弹层稳定盖住浮窗。
		 *
		 *  样式自足：--dpo-acc/--dpo-surface/--dpo-line/--dpo-ease 等定义在
		 *  `.dpo-controls,.dpo-pop,.dpo-overlay{...}` 三选择器上，--dpo-fs-* 同理 —— .dpo-pop 自己就被覆盖。
		 *  交互无关：closePops 纯 store 变更；popItemKeyNav 与外部点击关闭都用 closest(".dpo-pop") 相对查找；
		 *  初始聚焦用 document.querySelector(".dpo-pop") 全局查找；.dpo-pop 是 position:fixed，本就相对视口。
		 */
		function portalPop(el) {
			return el ? ReactDOM.createPortal(el, document.body) : el;
		}
		...
		/** 上游出处（BSD-3 要求保留）。这一行是给用户看的「这是什么插件」信息。 */
		const UPSTREAM = { author: "啃轮胎的西狐", version: "v0.1.1-beta.1", date: "2026/09/11" };
		/** 本 fork 的版本号。**必须与 package.json 的 version 一致** —— _sync.mjs 会断言，不一致会拦下同步。 */
		const PLUGIN_VERSION = "0.4.34";
/* v138 健壮性：单条待优化文本的字符上限（与宿主端 MAX_REQUEST_CHARS 必须一致）。
   超长输入会变成巨额 token 与超时，商业产品应在发起请求前就拦下并明确告知。 */
/* ══════════ v144 i18n：跟随 DSH 的界面语言 ══════════
   DSH 自己支持 zh / en（@deepseek-ai/dsh-client-locale 的 LOCALE_IDS），且它的
   FALLBACK_LOCALE 是 en —— 也就是说系统语言不是中文时 DSH 自己是英文界面，而插件
   原先 100% 硬编码中文，那正是"外挂感"最重的一处。

   设计：**以中文原文为 key 的英文词典**。
     · t() 在非英文环境**直接返回入参** ⇒ 中文界面逐字节不变（零回归风险）；
     · 未收录的字符串回落中文原文 ⇒ 翻译可以增量补齐，任何时刻都是可发布状态；
     · `\u0000` 之后的语境后缀只用于"同一中文串在不同位置需要不同译法"（如档位的
       「关闭」= Off，而弹层按钮的「关闭」= Close）。
   语言来源：ctx.locale.getLocale().active（见 syncLocale），存在 store.locale 上，
   纯运行时、不落盘。 */
const LOCALE_EN = "en";
const EN_DICT = {
	...
	/* ── 档位（TIERS） ── */
	"关闭\u0000tier": "Off",
	"原样发出，不拦截": "Send as-is, no interception",
	"完全不拦截，恢复原生发送": "No interception at all — native send restored",
	"普通": "Basic",
	"快速润色，约 3 秒": "Quick polish, about 3s",
	"只把话说清楚，不加新需求（约 3 秒）": "Just makes the wording clear; adds no new requirements (~3s)",
	"高级": "Advanced",
	"优化更完整，约 20 秒": "Fuller rewrite, about 20s",
	"补上显然需要的约束与验收（约 20 秒）": "Adds the constraints and acceptance criteria that are obviously needed (~20s)",
	"极端": "Extreme",
	"读项目消歧，约 20 秒以上": "Reads the project to disambiguate, 20s+",
	"读项目真实情况，仅用于确认指代（约 20 秒）": "Reads the real project, only to resolve references (~20s)",
	/* ── 发送档（PERMISSIONS） ── */
	"审查": "Review",
	"停在浮窗，等你确认再发出": "Stops in the panel and waits for you to confirm",
	"产出可编辑，点「确认提交」才发送": "The output is editable; it sends only when you click Send",
	"自动": "Auto",
	"优化完直接替你把消息发出": "Sends the message for you once optimized",
	"优化一完成就自动发出（失败也会按原文发出）": "Sends as soon as optimization finishes (falls back to your original text on failure)",
	/* ── 控件行 ── */
	"档位": "Tier",
	"发送": "Sending",
	"会话默认": "Session default",
	"提示词优化档位：{list}": "Prompt optimization tier: {list}",
	"优化完成后：先给你过目 / 直接发出": "After optimizing: let me review first / send directly",
	"优化所用模型（与对话模型独立）": "Model used for optimization (independent of the chat model)",
	"使用帮助（怎么用 / 档位 / 发送 / 迷你窗按钮）": "Help (how it works / tiers / sending / panel buttons)",
	/* ── 弹层 ── */
	"关闭": "Close",
	"提示词优化档位": "Prompt optimization tier",
	"优化完成后": "After optimizing",
	"优化模型": "Optimization model",
	"优化模型（与对话模型独立）": "Optimization model (independent of the chat model)",
	"使用帮助": "Help",
	"使用帮助 · 提示词优化": "Help · Prompt optimizer",
	"恢复默认（跟随会话）": "Reset to default (follows the session)",
	"刷新目录": "Refresh list",
	"正在加载模型目录…": "Loading model list…",
	"加载失败：": "Failed to load: ",
	"（暂无可用模型）": "(no models available)",
	"重试": "Retry",
	"目录为空": "The model list is empty",
	"会话当前": "Current session",
	"（当前会话模型）": " (current chat model)",
	/* ── 帮助面板 ── */
	"怎么用": "How it works",
	"照常输入，按 Enter（或点发送）": "Type as usual and press Enter (or click Send)",
	"消息不会直接发出，先被优化": "The message is not sent right away — it is optimized first",
	"迷你窗里看「思考/产出」，再决定发送": "Review thinking/output in the panel, then decide whether to send",
	"迷你窗按钮": "Panel buttons",
	"‹ 回退": "‹ Roll back",
	"停掉本次优化、不发消息，原文留在输入框（会先问一次）": "Stops this optimization without sending; your original text stays in the input (asks once first)",
	"复制": "Copy",
	"把结果、原文、思考、查证一起复制到剪贴板": "Copies the result, original text, thinking and citations to the clipboard",
	"确认提交": "Send",
	"按优化后的文本发出（上方可直接编辑）": "Sends the optimized text (you can edit it above)",
	"重新生成": "Regenerate",
	"先给个方向，再按该方向重跑一版": "Give a direction and re-run along it",
	"放行本条": "Send as-is",
	"不优化了，按原文直接发出（任何状态都可选）": "Skips optimization and sends your original text (available in any state)",
	"拖标题栏 / 右下角": "Drag the title bar / bottom-right corner",
	"移动窗口 / 改窗口大小（都会记住）": "Move the panel / resize it (both are remembered)",
	"隐私": "Privacy",
	"发送内容": "What is sent",
	"只发你在输入框里的这条文本 + 档位要求的内容": "Only the text in your input box, plus whatever the tier requires",
	"会话背景": "Session context",
	"「会话背景」开启时，额外发本会话最近的对话（仅用于补全指代；模型菜单里可关，关闭后不读取任何历史）": "When \"Session context\" is on, the recent conversation in this session is sent as well (only to resolve references; you can turn it off in the model menu — off reads no history at all)",
	"项目查证": "Project lookup",
	"只有【极端】档会读项目，且只在本项目根目录内只读": "Only the Extreme tier reads the project, and only read-only inside the project root",
	"本地日志": "Local logs",
	"默认不写任何日志：平时打字、拦截、切换档位都不会落盘或发请求（仅未捕获错误与自愈失败会留证）。日志只记长度、计数与状态，不含你的输入原文；单个超过 2MB 自动轮转，仅保留最近两代，全部保存在本机、不会上传": "No logs are written by default: typing, intercepting and switching tiers never touch the disk or the network (only uncaught errors and failed self-healing leave a trace). Logs contain lengths, counts and states only — never your input text. Each file rotates at 2MB and only the two most recent are kept; everything stays on this machine and is never uploaded",
	"清空日志": "Clear logs",
	"删除本机诊断日志（不影响设置、优化历史与任何会话内容）": "Delete local diagnostic logs (does not affect settings, optimization history or any session content)",
	"知道了": "Got it",
	"想让它读项目把指代写准并自动发送，建议【极端】+【自动】。": "To let it read the project, resolve references precisely and send automatically, use Extreme + Auto.",
	/* ── 迷你窗（浮窗） ── */
	"提示词优化 · 已拦截": "Prompt optimizer · intercepted",
	"↘ 可改大小": "↘ resizable",
	"发送：": "Sending: ",
	"档位 ": "Tier ",
	"你的原文": "Your original",
	"思考": "Thinking",
	"产出": "Output",
	"背景 {n} 轮": "{n} turns of context",
	"已把本会话最近 {n} 轮对话作为消歧背景提供给优化模型（可在模型菜单里关闭）": "The last {n} turns of this session were provided to the optimization model as disambiguation context (can be turned off in the model menu)",
	"回退：停掉本次优化并保留输入框原文": "Roll back: stops this optimization and keeps your original text in the input",
	"按住拖动（右下角可改大小）": "Drag to move (the bottom-right corner resizes)",
	"本次优化总 token": "Total tokens for this optimization",
	"本次思考消耗 token（provider 上报）": "Thinking tokens consumed (reported by the provider)",
	"复制全部内容": "Copy everything",
	"优化后的内容（可直接编辑）": "Optimized content (editable)",
	"按原文直接发出，不优化": "Send the original text as-is, without optimizing",
	"拖动改大小（记住）": "Drag to resize (remembered)",
	"确定回退？将停止优化、关闭浮层，且不发送任何消息。": "Roll back? This stops the optimization, closes the panel and sends nothing.",
	"确定回退": "Roll back",
	"取消回退": "Cancel",
	"已按 {tier} 档优化结果发送": "Sent the {tier} tier result",
	"已拦截 · 见浮窗": "Intercepted · see the panel",
	" · {n} 已拦截": " · {n} intercepted",
	"优化中 · 见浮窗": "Optimizing · see the panel",
	"输入过长（{n} 字，上限 {max}）· 请精简后重试": "Input too long ({n} chars, limit {max}) · please shorten it and retry",
	"正在加载模型目录…（稍后自动重试）": "Loading model list… (will retry automatically)",
	"（可拖动；当前：": " (draggable; current: ",
	"点击关闭：关闭后不再读取本会话的任何历史消息": "Click to turn off: no history from this session will be read",
	"已开启：最近的对话会作为消歧背景，仅用于补齐指代": "On: the recent conversation is used as disambiguation context, only to resolve references",
	"已关闭：不读取任何历史对话": "Off: no conversation history is read",
	"未知错误": "unknown error",
	"）": ")",
	"想让它读项目把指代写准并自动发送，建议【极端】+【自动】。": "Want it to read the project so references are accurate and send automatically? Use Extreme + Auto.",
	"上游 {author} {version}（{date}） · LessXi fork v{plugin}": "Upstream {author} {version} ({date}) · LessXi fork v{plugin}",
	"会话背景：把本会话最近的对话作为消歧背景提供给优化模型": "Session context: provides this session's recent conversation to the optimization model as disambiguation context",
	"本次会话累计拦截的发送次数": "Total sends intercepted in this session",
	"查证": "Verify",
	"收尾": "Wrap-up",
	"（暂无查证动作）": "(no verification actions yet)",
	"查证动作 {n} 步": "Verification: {n} steps",
	"（仅列前 8 步）": " (first 8 shown)",
	" · 已达轮次上限并收敛": " · round cap reached, converged",
	"{n}行": "{n} lines",
	"已回退：优化已停止，输入框原文保留": "Rolled back: optimization stopped, your text is back in the composer",
	"另一会话的优化已完成，切回该会话即自动发送": "Another session finished optimizing; switch back to it to send automatically",
	"启动失败：": "Failed to start: ",
	"优化模型不可用 → 已按原文发出，并回退到默认模型": "Optimization model unavailable → sent as-is and reverted to the default model",
	"优化失败 → 已按原文发出": "Optimization failed → sent as-is",
			"无法连接优化服务 · 本次优化中断，可重试": "Cannot reach the optimization service — this run was interrupted; you can retry",
			"优化服务不可用 → 已按原文发出": "Optimization service unavailable → sent your text as-is",
	"优化未产出内容 → 已按原文发出": "Optimization produced nothing → sent as-is",
	"该模型供应商未注册（没有可用适配器）——请在优化模型里换一个可用的 provider": "The model provider is not registered (no adapter available) — pick another provider from the optimization model menu",
	"请求已被取消（回退或超时）": "Request cancelled (rolled back or timed out)",
	"找不到可用的模型路由（请先选一个优化模型）": "No usable model route found (pick an optimization model first)",
	"凭据无效或未授权（请检查该 provider 的 API Key）": "Invalid or unauthorized credentials (check this provider's API key)",
	"请求过于频繁，请稍后重试": "Too many requests — please retry shortly",
	"请求超时，可重试": "Request timed out — you can retry",
	"提示词优化 · 渲染降级": "Prompt optimizer · degraded rendering",
	"已记录错误": "Error recorded",
	"浮层渲染出错，已降级为最小面板（优化仍在后台进行）。错误：": "Panel render error — fell back to a minimal panel (optimization continues in the background). Error: ",
	"未知": "unknown",
	"（等待{what}…）": "({what} pending…)",
	"（优化中断，无{what}内容）": "(optimization stopped — no {what})",
	"（本次没有{what}内容）": "(no {what} this time)",
	"优化中…": "Optimizing…",
	"清理中…": "Clearing…",
	"已完成": "Done",
	"失败": "Failed",
	" · 首字 {ms}ms": " · first token {ms}ms",
	"该 provider 未上报思考 token": "This provider does not report thinking tokens",
	"产出的输出 token": "Output tokens",
	"产出字数": "Output length",
	"思考字数": "Thinking length",
	"{n} 字": "{n} chars",
	"失败：": "Failed: ",
	"已复制": "Copied",
	"复制失败": "Copy failed",
	"以下内容将原样发送（可直接编辑） · {n} 字": "The following will be sent as-is (editable) · {n} chars",
	"重新生成 · 给个方向（可留空）": "Regenerate · give a direction (optional)",
	"重新生成的方向（可留空）": "Direction for regeneration (optional)",
	"例如：更短、保留技术细节、强调验收标准…": "e.g. shorter, keep the technical details, emphasise acceptance criteria…",
	"回车即可重跑 · 留空则随机换一次方向": "Press Enter to re-run · leave empty for a random direction",
	"按此方向重跑": "Re-run with this direction",
	"取消": "Cancel",
	"提示词优化浮窗": "Prompt optimizer panel",
	"跳到查证轨迹": "Jump to lookup trace",
	"查证 {n} 步 ↓": "{n} lookup steps ↓",
	"优化进行中 · 你的输入已保留在输入框，可稍候或点「回退」": "Optimizing · your input is kept in the input box; wait or click \"Roll back\"",
	"上一次的结果还没确认 · 你的输入已保留在输入框，请先「确认提交」或「回退」": "The previous result is unconfirmed · your input is kept in the input box; click \"Send\" or \"Roll back\" first",
	"不可达": "unreachable",
	"（{n}）": " ({n})",
	"档位为「关闭」时不生效": "Has no effect while the tier is \"Off\"",
	"点击开启：只读取本会话最近的对话，仅用于消歧": "Click to turn on: only the recent conversation in this session is read, purely to resolve references",
	"已清空 {n} 项": "Cleared {n}",
	"本地诊断日志已清空（{n} 项 · {kb} KB）": "Local diagnostic logs cleared ({n} files · {kb} KB)",
	"本地诊断日志已清空（本来就没有日志）": "Local diagnostic logs cleared (there were none)",
	"重启后可用": "Available after restart",
	"清空失败": "Clearing failed",
	"清空日志失败：": "Failed to clear logs: ",
	"清空日志需要先重启一次 DSH —— 宿主端更新只有在重启后生效": "Clearing logs needs a DSH restart first — host-side updates only take effect after a restart",
	"等待 provider 上报用量": "Waiting for the provider to report usage",
	/* ── v154 诊断取证开关 ── */
	"诊断取证": "Diagnostics capture",
	"诊断取证：把输入与结果的前几十字写进本机诊断日志，便于排查问题": "Diagnostics capture: writes the first few dozen characters of your input and results to the local diagnostic log, to help with troubleshooting",
	"点击关闭：不再往日志里写任何原文片段": "Click to turn off: no fragments of your text will be written to the log",
	"点击开启：会记录输入与结果的前几十字，仅保存在本机": "Click to turn on: the first few dozen characters of your input and results are recorded, on this machine only",
	"已开启：输入与结果的前几十字会写进本机日志（不会上传）": "On: the first few dozen characters of your input and results go to the local log (never uploaded)",
	"已关闭：日志不含任何原文": "Off: the log contains no fragments of your text",
	"设置同步": "Settings sync",
	"失败：你的插件设置仍保存在本机，但写不进 DSH 的设置文件（多见于上次异常退出留下的锁文件）。删除 settings.yaml.lock，或重启一次 DSH，即可恢复": "Failed: your plugin settings are still saved on this machine, but they cannot be written into the DSH settings file (usually a lock file left behind by an unclean exit). Delete settings.yaml.lock, or restart DSH once, to recover",
	"诊断取证已开启：输入与结果的前几十字会写进本机日志（开关在模型菜单底部）。日志只存本机、不会上传，超 2MB 自动轮转，只留最近两代": "Diagnostics capture is ON: the first few dozen characters of your input and results go to the local log (switch at the bottom of the model menu). Logs stay on this machine and are never uploaded; they rotate at 2 MB, keeping the two most recent files",
	"诊断取证已关闭：日志只记长度、计数与状态，不含你的输入原文（开关在模型菜单底部）。日志只存本机、不会上传，超 2MB 自动轮转，只留最近两代": "Diagnostics capture is OFF: the log records only lengths, counts and status — never your text (switch at the bottom of the model menu). Logs stay on this machine and are never uploaded; they rotate at 2 MB, keeping the two most recent files",
};
/* 取英文文案：s = 中文原文（也是 key）；ctx = 可选语境后缀；vars = {占位符} 取值。
   非英文环境原样返回 s —— 这是"中文界面零变化"的唯一保证点。 */
function t(s, vars, ctx) {
	let out = s;
	if (store.locale === LOCALE_EN) {
		const key = ctx ? s + "\u0000" + ctx : s;
		const hit = Object.prototype.hasOwnProperty.call(EN_DICT, key) ? EN_DICT[key] : EN_DICT[s];
		if (typeof hit === "string") out = hit;
	}
	if (vars) out = out.replace(/\{(\w+)\}/g, (m, k) => (vars[k] === undefined ? m : String(vars[k])));
	return out;
}
/* 读 DSH 的当前语言写进 store.locale。拿不到（无 locale 服务/异常）就保持中文，
   等于旧行为 —— 这是"环境识别失败时绝不比现在更差"的兜底。 */
function syncLocale() {
	try {
		const snap = localeService && typeof localeService.getLocale === "function" ? localeService.getLocale() : null;
		const id = snap && typeof snap.active === "string" ? snap.active : null;
		if (id && id !== store.locale) {
			store.locale = id;
			return true;
		}
	} catch (e) { /* noop */ }
...
const MAX_REQUEST_CHARS = 32000;
		...
		const TIERS = [
			/* v144：label / hint / help 改为 getter —— 所有读取点（控件行标签、弹层项、
			   帮助面板、状态行、浮窗 src 行、发送通知）零改动即可跟随语言切换。 */
			{ id: "off", get label() { return t("关闭", null, "tier"); }, get hint() { return t("原样发出，不拦截"); }, get help() { return t("完全不拦截，恢复原生发送"); } },
			{ id: "basic", get label() { return t("普通"); }, get hint() { return t("快速润色，约 3 秒"); }, get help() { return t("只把话说清楚，不加新需求（约 3 秒）"); } },
			{ id: "advanced", get label() { return t("高级"); }, get hint() { return t("优化更完整，约 20 秒"); }, get help() { return t("补上显然需要的约束与验收（约 20 秒）"); } },
			{ id: "extreme", get label() { return t("极端"); }, get hint() { return t("读项目消歧，约 20 秒以上"); }, get help() { return t("读项目真实情况，仅用于确认指代（约 20 秒）"); } },
		];
		const PERMISSIONS = [
			{ id: "review", get label() { return t("审查"); }, get hint() { return t("停在浮窗，等你确认再发出"); }, get help() { return t("产出可编辑，点「确认提交」才发送"); } },
			{ id: "auto", get label() { return t("自动"); }, get hint() { return t("优化完直接替你把消息发出"); }, get help() { return t("优化一完成就自动发出（失败也会按原文发出）"); } },
		];
		let noticeTimer = 0;
		function showNotice(text) {
			/* v134：toast 与 .dpo-pop 同因 —— 留在 composer 子树里会被 .wSkVaW_composerSeat
			   {position:sticky;z-index:7} 关进有效层级 7 的层叠上下文，而浮窗在 .pI_x6G_overlayLayer
			   {z-index:20} 内（有效层级 20）。实测：浮窗拖到下半屏（clampPos 允许 maxY=302）后
			   toast 中心 elementFromPoint 命中 TEXTAREA.dpo-review-text 而非自身。
			   portal 到 body 后 position:fixed 需自算坐标：贴控件行上方 8px、左对齐控件行左边。 */
			let anchor = null;
			try {
				const ctl = document.querySelector(".dpo-controls");
				const r = ctl ? ctl.getBoundingClientRect() : null;
				if (r) anchor = { left: Math.round(r.left), bottom: Math.round(window.innerHeight - r.top + 8) };
			} catch (e) { /* best effort */ }
			store.notice = { text, until: Date.now() + 2600, anchor };
			emit();
		...
		function persistState(patch) {
			try {
				// v153：顺带回读响应里的遥测开关。客户端从不发送 debugEvidence（避免用陈旧值覆盖），
				// 而设置面板改它只写宿主侧文件 —— 借这次已有的往返把它同步过来，否则必须刷新页面才生效。
				fetch(API + "/state", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(Object.assign({ tier: store.tier, permission: store.permission, model: store.modelSel, sessionId: store.viewSessionId || null, useContext: store.useContext !== false }, patch || {})) })
					.then((r) => (r.ok ? r.json() : null))
					.then((d) => {
						const st = d && d.ok ? d.state : null;
						if (st && typeof st.debugEvidence === "boolean" && st.debugEvidence !== store.debugEvidence) store.debugEvidence = st.debugEvidence;
						if (d && typeof d.settings === "string") {
							const sync = d.settings.indexOf("mirror-error") >= 0 ? "error" : "ok";
							if (sync !== store.settingsSync) { store.settingsSync = sync; emit(); }
						}
					})
					.catch(() => {});
			} catch (e) { /* best effort */ }
		}
		/** v140 · 用户隐私控制面：清空本机 evidence 目录下的诊断日志。
		 *  宿主端 lib/index.js 无 watch，旧进程没有该路由会返回 404 —— 此时明确提示
		 *  「重启 DSH 后可用」，而不是静默失败（用户点了没反应的按钮等于没有按钮）。 */
		function clearLogs() {
			if (store.clearLogsBusy === true) return;
			store.clearLogsBusy = true;
			store.clearLogsLabel = t("清理中…");
			emit();
			const settle = (label, text, ms) => {
				store.clearLogsBusy = false;
				store.clearLogsLabel = label;
				emit();
				if (text) showNotice(text);
				window.setTimeout(() => { store.clearLogsLabel = null; emit(); }, ms || 3000);
			};
			try {
				fetch(API + "/evidence/clear", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" })
					.then((r) => (r.ok ? r.json() : Promise.reject(new Error("http-" + r.status))))
					.then((d) => {
						const n = d && Array.isArray(d.removed) ? d.removed.length : 0;
						const kb = d && d.bytesFreed ? Math.max(1, Math.round(d.bytesFreed / 1024)) : 0;
						beacon("logs-cleared", { removed: n, bytes: d ? d.bytesFreed : null });
						settle(t("已清空 {n} 项", { n: n }), n ? t("本地诊断日志已清空（{n} 项 · {kb} KB）", { n: n, kb: kb }) : t("本地诊断日志已清空（本来就没有日志）"), 3200);
					})
					.catch((err) => {
						const s = String((err && err.message) || err);
						const stale = s.indexOf("http-404") >= 0 || s.indexOf("http-405") >= 0;
						beacon("logs-clear-failed", { reason: s });
						settle(stale ? t("重启后可用") : t("清空失败"), stale ? t("清空日志需要先重启一次 DSH —— 宿主端更新只有在重启后生效") : t("清空日志失败：") + s, 3600);
					});
			} catch (e) {
				settle(t("清空失败"), t("清空日志失败：") + String((e && e.message) || e), 3200);
			}
		}
				...
				window.setTimeout(() => {
					try {
						if (store.latest.actions && store.latest.sessionId === sid) {
							...
							showNotice(t("已按 {tier} 档优化结果发送", { tier: (TIERS.find((x) => x.id === run.tier) || {}).label || run.tier }));
							emit();
						}
					} catch (e) { /* noop */ }
				}, 320);
		...
		function insideComposer() {
			...
			return true;
				...
				/* v139 · 遥测出口统一脱敏：诊断日志不该成为用户输入的留底。
		   只在这一个出口处理，68 处调用点零改动；被改写的字符串补记 *Len 原长，保留可诊断性。 */
		const SECRET_RE = [
			/\b(?:sk|pk|rk|ghp|gho|ghs|ghu|github_pat|xox[baprs]|AKIA|AIza)[-_][A-Za-z0-9_-]{12,}/g,
		];
		/* 高熵串只在「整串」上判定：长度 >=40、只含 base64-ish 字符、且大小写与数字齐备。
		   早期版本对任何 40+ 字母数字串做全局替换，会把普通长文本（如 300 个 x）也刷成
		   <secret>，等于毁掉遥测的诊断价值 —— 实测抓出该误伤后改为保守判定。 */
		function looksLikeSecret(s) {
			if (s.length < 40) return false;
			if (!/^[A-Za-z0-9+/=_-]+$/.test(s)) return false;
			return /[A-Z]/.test(s) && /[a-z]/.test(s) && /\d/.test(s);
		}
		function sanitizeBeaconValue(v) {
			let s = String(v);
			s = s.replace(/[A-Za-z]:\\[^\s"'<>|]*/g, "<path>");
			s = s.replace(/\/(?:home|Users|root|var|tmp|opt|usr|mnt|srv)\/[^\s"'<>|]*/g, "<path>");
			for (const re of SECRET_RE) s = s.replace(re, "<secret>");
			if (looksLikeSecret(s)) s = "<secret>";
			if (s.length > 120) s = s.slice(0, 120) + "\u2026";
			return s;
		}
		function sanitizeBeaconData(data) {
			const out = {};
			for (const k of Object.keys(data || {})) {
				const v = data[k];
				if (typeof v === "string") {
					const c = sanitizeBeaconValue(v);
					out[k] = c;
					if (c !== v) out[k + "Len"] = v.length;
				} else if (Array.isArray(v)) {
					out[k] = v.map((x) => (typeof x === "string" ? sanitizeBeaconValue(x) : x));
				} else {
					out[k] = v;
				}
			}
...
function beacon(stage, data) {
			/* v141：默认静默。只有故障信号/用户可见失败，或用户在 /state 里开启 debugEvidence 时才上报。 */
			if (store.debugEvidence !== true && !ALWAYS_BEACON.has(stage)) return;
			try {
				fetch(API + "/beacon", {
					method: "POST",
					headers: { "content-type": "application/json" },
					body: JSON.stringify(Object.assign({ t: Date.now(), stage }, sanitizeBeaconData(data))),
				}).catch(() => {});
			} catch (e) { /* best effort */ }
		}
		...
		function Slider(props) {
			...
			return h("div", {
				...
				title: (props.title || "") + t("（可拖动；当前：") + ((props.options[idx] || {}).label || "") + t("）"),
			},
				h("div", {
		...
		function helpPopover() {
			...
			/* v144：帮助面板是全插件文案最密集处（48 行），在这里单点翻译 —— 键与值都过 t()，
			   调用点零改动。第三个参数是 React key（英文 id），不翻译。 */
			const line = (k, v, id) => h("div", { className: "dpo-help-row", key: id }, h("span", { className: "dpo-help-k" }, t(k)), h("span", { className: "dpo-help-v" }, t(v)));
			return h("div", {
				className: "dpo-pop dpo-help-pop", "data-dpo": "help-pop", role: "dialog", "aria-label": t("使用帮助"),
				style: { left: pos.x + "px", bottom: (pos.bottom || 200) + "px", maxHeight: (pos.maxH || 420) + "px" },
			},
				h("div", { className: "dpo-pop-head" }, t("使用帮助 · 提示词优化")),
				h("div", { className: "dpo-help-sec" }, t("怎么用")),
				line("①", "照常输入，按 Enter（或点发送）"),
				line("②", "消息不会直接发出，先被优化"),
				line("③", "迷你窗里看「思考/产出」，再决定发送"),
				h("div", { className: "dpo-help-sec" }, t("档位")),
				...TIERS.map((t) => line(t.label, t.help, t.id)),
				h("div", { className: "dpo-help-sec" }, t("发送")),
				...PERMISSIONS.map((p) => line(p.label, p.help, p.id)),
				h("div", { className: "dpo-help-sec" }, t("迷你窗按钮")),
				line("‹ 回退", "停掉本次优化、不发消息，原文留在输入框（会先问一次）"),
				line("复制", "把结果、原文、思考、查证一起复制到剪贴板"),
				line("确认提交", "按优化后的文本发出（上方可直接编辑）"),
				line("重新生成", "先给个方向，再按该方向重跑一版"),
				line("放行本条", "不优化了，按原文直接发出（任何状态都可选）"),
				line("拖标题栏 / 右下角", "移动窗口 / 改窗口大小（都会记住）"),
				h("div", { className: "dpo-help-sec" }, t("隐私")),
				line("发送内容", "只发你在输入框里的这条文本 + 档位要求的内容"),
				line("会话背景", "「会话背景」开启时，额外发本会话最近的对话（仅用于补全指代；模型菜单里可关，关闭后不读取任何历史）"),
				line("项目查证", "只有【极端】档会读项目，且只在本项目根目录内只读"),
				line("本地日志", store.debugEvidence === true
					? t("诊断取证已开启：输入与结果的前几十字会写进本机日志（开关在模型菜单底部）。日志只存本机、不会上传，超 2MB 自动轮转，只留最近两代")
					: t("诊断取证已关闭：日志只记长度、计数与状态，不含你的输入原文（开关在模型菜单底部）。日志只存本机、不会上传，超 2MB 自动轮转，只留最近两代")),
				// v154：设置文档写不进去时明说 —— 静默失败等于用户永远不知道自己「改了没生效」
				store.settingsSync === "error" ? line("设置同步", t("失败：你的插件设置仍保存在本机，但写不进 DSH 的设置文件（多见于上次异常退出留下的锁文件）。删除 settings.yaml.lock，或重启一次 DSH，即可恢复")) : null,
				h("div", { className: "dpo-help-tip", "data-dpo": "help-tip" },
					t("想让它读项目把指代写准并自动发送，建议【极端】+【自动】。"),
				),
				h("div", { className: "dpo-pop-foot" },
					h("button", {
						type: "button", className: "dpo-btn dpo-clear-logs", "data-dpo": "clear-logs",
						"aria-disabled": store.clearLogsBusy === true,
						title: t("删除本机诊断日志（不影响设置、优化历史与任何会话内容）"),
						onClick: clearLogs,
					}, store.clearLogsLabel || t("清空日志")),
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "help-close", onClick: () => { store.helpOpen = false; emit(); } }, t("知道了")),
				),
				// 署名（面板最底部）
				h("div", { className: "dpo-help-meta", "data-dpo": "help-meta" },
					t("上游 {author} {version}（{date}） · LessXi fork v{plugin}", { author: UPSTREAM.author, version: UPSTREAM.version, date: UPSTREAM.date, plugin: PLUGIN_VERSION })),
			);
		}
		...
		function tierPopover() {
			...
			return h("div", {
				className: "dpo-pop", "data-dpo": "tier-pop", role: "menu", "aria-label": t("提示词优化档位"),
				style: { left: pos.x + "px", bottom: pos.bottom + "px", maxHeight: pos.maxH + "px" },
			},
				h("div", { className: "dpo-pop-head" }, t("提示词优化档位")),
				...
				h("div", { className: "dpo-pop-foot" },
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "tier-close", onClick: () => { store.tierPopOpen = false; emit(); } }, t("关闭")),
				),
			);
		}
		...
		function permPopover() {
			...
			return h("div", {
				className: "dpo-pop", "data-dpo": "perm-pop", role: "menu", "aria-label": t("优化完成后"),
				style: { left: pos.x + "px", bottom: pos.bottom + "px", maxHeight: pos.maxH + "px" },
			},
				h("div", { className: "dpo-pop-head" }, t("优化完成后")),
				h("div", { className: "dpo-pop-group" },
				...
				h("div", { className: "dpo-pop-foot" },
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "perm-close", onClick: () => { store.permPopOpen = false; emit(); } }, t("关闭")),
				),
			);
		}
		...
		function modelPopover() {
			...
			return h("div", {
				className: "dpo-pop", "data-dpo": "model-pop", role: "menu", "aria-label": t("优化模型"),
				style: { left: pos.x + "px", bottom: (pos.bottom || 200) + "px", maxHeight: (pos.maxH || 320) + "px" },
				onMouseLeave: () => { /* 保持打开，点击外部关闭 */ },
			},
				h("div", { className: "dpo-pop-head" }, t("优化模型（与对话模型独立）")),
				store.modelCatalogLoading === true && groups.length === 0
					? h("div", { className: "dpo-pop-empty", "data-dpo": "model-loading" }, t("正在加载模型目录…"))
					: null,
				groups.length === 0 && store.modelCatalogLoading !== true
					? h("div", { className: "dpo-pop-empty", "data-dpo": "model-error" },
						(store.modelCatalogError ? t("加载失败：") + t(store.modelCatalogError) : t("（暂无可用模型）")),
						h("button", { type: "button", className: "dpo-btn", "data-dpo": "model-retry", onClick: () => loadCatalog("retry", true) }, t("重试")),
					)
					: null,
				...groups.map((g) => h("div", { key: g.id, className: "dpo-pop-group", "data-dpo": "model-group" },
					h("div", { className: "dpo-pop-gtitle", "data-dpo": "model-group-name" }, g.name + t("（{n}）", { n: (g.models || []).length }) + (g.degraded ? " · " + t("不可达") : "")),
					...(g.models || []).map((m) => {
						...
						return h("button", {
							...
							title: g.name + " · " + m.name + (cur ? t("（当前会话模型）") : ""),
							onClick: () => { store.modelSel = { provider: g.id, model: m.id, name: m.name }; store.modelPopOpen = false; persistState(); emit(); },
							onKeyDown: popItemKeyNav,
						}, m.name, cur && !sel ? h("span", { className: "dpo-pop-chip" }, t("会话当前")) : null);
					}),
				)),
				h("div", { className: "dpo-pop-foot" },
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "model-reset", onClick: () => { store.modelSel = null; store.modelPopOpen = false; persistState({ tier: store.tier, permission: store.permission, model: null }); emit(); } }, t("恢复默认（跟随会话）")),
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "model-refresh", onClick: () => loadCatalog("refresh", true) }, t("刷新目录")),
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "model-close", onClick: () => { store.modelPopOpen = false; emit(); } }, t("关闭")),
				),
				h("div", { className: "dpo-pop-priv", "data-dpo": "ctx-row" },
					h("button", {
						type: "button", className: "dpo-switch", "data-dpo": "ctx-toggle",
						role: "switch", "aria-checked": String(store.useContext !== false),
						"data-on": String(store.useContext !== false),
						"aria-label": t("会话背景：把本会话最近的对话作为消歧背景提供给优化模型"),
						title: store.useContext !== false ? t("点击关闭：关闭后不再读取本会话的任何历史消息") : t("点击开启：只读取本会话最近的对话，仅用于消歧"),
						onClick: () => { store.useContext = !(store.useContext !== false); store.useContextTouched = true; persistState({ useContext: store.useContext }); emit(); },
					},
						h("span", { className: "dpo-switch-track", "aria-hidden": "true" },
							h("span", { className: "dpo-switch-knob" }),
						),
						h("span", { className: "dpo-switch-label" }, t("会话背景")),
					),
					h("span", { className: "dpo-pop-priv-hint", "data-dpo": "ctx-hint" },
						store.useContext !== false
							? t("已开启：最近的对话会作为消歧背景，仅用于补齐指代")
							: t("已关闭：不读取任何历史对话"),
					),
					h("button", {
						type: "button", className: "dpo-switch", "data-dpo": "diag-toggle",
						role: "switch", "aria-checked": String(store.debugEvidence === true),
						"data-on": String(store.debugEvidence === true),
						"aria-label": t("诊断取证：把输入与结果的前几十字写进本机诊断日志，便于排查问题"),
						title: store.debugEvidence === true ? t("点击关闭：不再往日志里写任何原文片段") : t("点击开启：会记录输入与结果的前几十字，仅保存在本机"),
						onClick: () => { store.debugEvidence = !(store.debugEvidence === true); store.debugEvidenceTouched = true; persistState({ debugEvidence: store.debugEvidence }); emit(); },
					},
						h("span", { className: "dpo-switch-track", "aria-hidden": "true" },
							h("span", { className: "dpo-switch-knob" }),
						),
						h("span", { className: "dpo-switch-label" }, t("诊断取证")),
					),
					h("span", { className: "dpo-pop-priv-hint", "data-dpo": "diag-hint" },
						store.debugEvidence === true
							? t("已开启：输入与结果的前几十字会写进本机日志（不会上传）")
							: t("已关闭：日志不含任何原文"),
					),
				),
			);
		...
		function Controls(props) {
			...
			React.useEffect(() => {
				...
				// 焦点管理：关闭时把焦点还给触发按钮。DSH 实测「设置面板打开后焦点在 VOzbGW_close，
				// 关闭后回到 VOzbGW_trigger」—— 从不把焦点丢给 BODY；弹层此前关闭后 activeElement 变 BODY。
				// 这里按弹层类型反查按钮而不是读 document.activeElement：从 A 弹层切到 B 弹层时，
				// 旧弹层被卸载会先把焦点丢给 BODY，紧随其后的 cleanup 又会把焦点放到旧按钮上。
				const popKind = store.tierPopOpen ? "tier" : store.permPopOpen ? "perm" : store.modelPopOpen ? "model" : "help";
				const returnTo = document.querySelector('[data-dpo="' + popKind + '"]');
				// 键盘可及：弹层打开后把初始焦点交给当前选中项。
				// 注：实测 DSH 原生菜单打开后焦点仍留在触发按钮（不移入），此处更主动 —— 让方向键立即可用；
				// DSH 的设置面板则会移焦（到 VOzbGW_close），两者的共同点是「绝不把焦点丢给 BODY」。
				try {
					...
					if (pop) {
						...
						const cur = pop.querySelector('.dpo-pop-item[data-selected="true"]') || items[0]
							|| pop.querySelector(".dpo-pop-foot .dpo-btn") || pop.querySelector("button");
				...
				return () => {
					...
					// 关闭弹层：把焦点还给触发按钮（元素仍在文档内才还，避免还给已被卸载的节点）。
					// 若还有弹层开着，说明只是 A→B 切换，此时不动焦点，交给新实例自己处理。
					if (store.tierPopOpen || store.permPopOpen || store.modelPopOpen || store.helpOpen) return;
					try {
			...
			return h("div", { ref: nodeRef, className: "dpo-controls", "data-dpo": "controls" },
						...
						title: t("提示词优化档位：{list}", { list: TIERS.map((x) => x.label).join(" / ") }),
						...
						h("span", { className: "dpo-model-k" }, t("档位")),
				...
				h("button", {
					...
					"data-off": String(off),
					title: off ? t("档位为「关闭」时不生效") : t("优化完成后：先给你过目 / 直接发出"),
					...
					h("span", { className: "dpo-model-k" }, t("发送")),
				...
				h("button", {
					...
					h("span", { className: "dpo-model-v" }, store.modelSel ? store.modelSel.name : t(store.modelLabel)),
					h("span", { className: "dpo-model-caret" }, "▾"),
				),
				h("button", {
					...
					onClick: (e) => {
						...
						/* v154：打开帮助时主动拉一次最新同步状态。POST /state 的响应在 mirrorToSettings
						   的 Promise settle 之前就返回（写锁超时要 2 秒才失败），只靠落盘回读会永远滞后一拍，
						   「设置同步」警告行因此永远不出现。打开面板这一下正好拿到已经 settle 的真实结果。 */
						if (willOpen) {
							fetch(API + "/state", { cache: "no-store" }).then((r) => r.json()).then((d) => {
								if (!d || typeof d.settings !== "string") return;
								const sync = d.settings.indexOf("mirror-error") >= 0 ? "error" : "ok";
								if (sync !== store.settingsSync) { store.settingsSync = sync; emit(); }
							}).catch(() => {});
						}
				...
				portalPop(helpPopover()),
				store.notice && Date.now() < store.notice.until
					? portalPop(h("span", {
						className: "dpo-notice dpo-notice-fixed",
						"data-dpo": "notice",
						"aria-hidden": "true",
						style: store.notice.anchor ? { left: store.notice.anchor.left + "px", bottom: store.notice.anchor.bottom + "px" } : null,
					}, store.notice.text))
					: null,
				portalPop(tierPopover()),
				portalPop(permPopover()),
				portalPop(modelPopover()),
				n > 0 ? h("span", { className: "dpo-count", "data-dpo": "count", title: t("本次会话累计拦截的发送次数") }, String(n)) : null,
				// 常驻屏读播报：role=status 隐式 aria-live=polite + aria-atomic=true
				// 与浮窗内的 .dpo-run-status（aria-live=polite）分工：这里播报浮窗外的瞬时通知
				h("span", { className: "dpo-sr-only", role: "status", "data-dpo": "a11y-status" },
					store.notice && Date.now() < store.notice.until ? store.notice.text : ""),
			);
		}

		function traceRows() {
			// 真实运行的查证记录随 SSE 到 run.trace；探针通道的 store.trace 只作兜底（自检路径）
			const live = store.run ? (store.run.trace || { rows: [] }) : null;
			const trc = live || store.trace;
			if (!trc) return null;
			const rows = live
				? (trc.rows || []).map((x) => ({ ...x, phase: t("查证") }))
				: [...(trc.normal || []).map((x) => ({ ...x, phase: t("查证") })), ...(trc.capped || []).map((x) => ({ ...x, phase: t("收尾") }))];
			if (rows.length === 0) {
				// v130：查证动作只可能出现在【极端】档；其它档位的空态面板是零信息（用户对"查证"无对照物）
				if (live && store.run && store.run.tier !== "extreme") return null;
				return h("div", { className: "dpo-trace", "data-dpo": "trace" }, h("div", { className: "dpo-trace-empty" }, t("（暂无查证动作）")));
			}
			return h("div", { className: "dpo-trace", "data-dpo": "trace" },
				h("div", { className: "dpo-trace-head" }, t("查证动作 {n} 步", { n: rows.length }) + (rows.length > 8 ? t("（仅列前 8 步）") : "") + (trc && trc.converged ? t(" · 已达轮次上限并收敛") : "")),
				...rows.slice(0, 8).map((r, i) => h("div", { key: i, className: "dpo-trace-row", "data-dpo": "trace-row" },
					...
					h("span", { className: "dpo-trace-meta" }, String(r.ms) + "ms · " + t("{n}行", { n: r.resultLines })),
				)),
			);
		}
		...
		function rollbackYes() {
			...
			showNotice(t("已回退：优化已停止，输入框原文保留"));
			emit();
		}
		...
		function interceptAndOptimize(text) {
			...
			if (busy || awaitingReview) {
				...
				showNotice(busy ? t("优化中 · 见浮窗") : t("已拦截 · 见浮窗"));
		...
		function startRun(request, tier, forceError, opts) {
			const extra = opts || {};
			// v138：超长文本在「开窗 + 发请求」之前就拦下（否则会白开一个浮窗、白跑一次 LLM）
			const reqText = String(request || "");
			if (reqText.length > MAX_REQUEST_CHARS) {
				beacon("request-too-long", { chars: reqText.length, max: MAX_REQUEST_CHARS, tier: String(tier || "") });
				showNotice(t("输入过长（{n} 字，上限 {max}）· 请精简后重试", { n: reqText.length, max: MAX_REQUEST_CHARS }));
				return null;
			}
			...
			const run = {
				...
				sessionId: sid, readyToSend: null, disposed: false, recoveryTries: 0,
			};
			store.run = run;
			store.reviewText = null;
			store.regenAsk = false;
			store.rollbackConfirm = false;   // v117：与 regenAsk 同族，漏了它会让旧的「确定回退」跟到新 run 上
			...
			const autoSend = (text, noticeText) => {
				...
				showNotice(t("另一会话的优化已完成，切回该会话即自动发送"));
				return false;
			};
			fetch(API + "/run", {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({ request, tier, sessionId: sid || store.latest.sessionId, forceError: forceError === true, useContext: store.useContext !== false, provider: store.modelSel ? store.modelSel.provider : null, model: store.modelSel ? store.modelSel.model : null, direction: extra.direction || null, prevText: extra.prevText || null }),
			}).then((r) => r.json()).then((d) => {
				if (!d || d.ok !== true) { run.status = "error"; run.error = t("启动失败：") + JSON.stringify(d); emit(); return; }
				run.runId = d.runId;
				run.contextTurns = typeof d.contextTurns === "number" ? d.contextTurns : 0;
				run.contextReason = d.contextReason || null;
				run.contextChars = typeof d.contextChars === "number" ? d.contextChars : 0;
				...
				es.onmessage = (ev) => {
					...
					if (msg.type === "snapshot") {
					...
					} else if (msg.type === "text-reset") {
						cur.text = "";
						emit();
					} else if (msg.type === "trace") {
						cur.trace = { rows: Array.isArray(msg.rows) ? msg.rows : [], capped: msg.capped === true, converged: msg.converged === true };
						emit();
						...
						if (store.permission === "auto") {
							...
							autoSend(original, deadRoute ? t("优化模型不可用 → 已按原文发出，并回退到默认模型") : t("优化失败 → 已按原文发出"));
						...
						if (store.permission === "auto" && String(cur.text || "").trim()) {
							autoSend(String(cur.text), t("已按 {tier} 档优化结果发送", { tier: cur.tier }));
						} else if (store.permission === "auto") {
							// 空产出也放行原文：宁可按原文发出，也不要"按了发送却什么都没发生"
							const original = String(cur.request || "");
							autoSend(original, t("优化未产出内容 → 已按原文发出"));
				...
				es.onerror = () => {
	// 断线：EventSource 会自行重连；但宿主完全不可达时永远收不到任何事件，
	// 状态会永久停在 running —— 拦截判定把 running 当 busy，插件从此不再拦截任何消息。
	// 这里做有界探活：宿主还活着就交给自动重连（会补发 snapshot 或 run-not-found），
	// 探不通就把本次运行判为中断，恢复全部出口并按档位 fail-open。
	if (run.disposed) return;
	if (run.status !== "running" && run.status !== "connecting") return;
	if (run.recoveryTries >= 3) return;
	run.recoveryTries = (run.recoveryTries || 0) + 1;
	const tries = run.recoveryTries;
	window.setTimeout(() => {
		if (run.disposed) return;
		if (run.status !== "running" && run.status !== "connecting") return;
		if (run.es && run.es.readyState === 1) return; // 已重连：不误判长思考
		let settled = false;
		const lost = t("无法连接优化服务 · 本次优化中断，可重试");
		const markLost = () => {
			if (settled) return;
			settled = true;
			run.status = "error";
			run.error = lost;
			beacon("run-host-lost", { tries: tries, runId: run.runId, permission: store.permission });
			if (store.permission === "auto") autoSend(String(run.request || ""), t("优化服务不可用 → 已按原文发出"));
			emit();
		};
		const to = window.setTimeout(markLost, 4000);
		fetch(API + "/state").then((res) => {
			window.clearTimeout(to);
			if (settled) return;
			// 宿主还活着且仍有重试额度：交给 EventSource 自动重连（会补 snapshot 或 run-not-found）
			if (res && res.ok && tries < 3) return;
			markLost();
		}).catch(() => {
			window.clearTimeout(to);
			markLost();
		...
		function humanizeError(raw) {
			const s = String(raw || "");
			if (s.includes("NO_ADAPTER")) return t("该模型供应商未注册（没有可用适配器）——请在优化模型里换一个可用的 provider");
			if (s.includes("ABORTED")) return t("请求已被取消（回退或超时）");
			if (s.includes("no-llm-route")) return t("找不到可用的模型路由（请先选一个优化模型）");
			if (/401|unauthor/i.test(s)) return t("凭据无效或未授权（请检查该 provider 的 API Key）");
			if (/429|rate.?limit/i.test(s)) return t("请求过于频繁，请稍后重试");
			if (/timeout|ETIMEDOUT/i.test(s)) return t("请求超时，可重试");
		...
		function runPanes() {
			...
			const emptyOf = (what) => { const w = t(what); return (live ? t("（等待{what}…）", { what: w }) : run.status === "error" ? t("（优化中断，无{what}内容）", { what: w }) : t("（本次没有{what}内容）", { what: w })); };
			const chips = usageChips(run.usage);
			const label = run.status === "running" || run.status === "connecting" ? t("优化中…")
				: run.status === "done" ? t("已完成") : run.status === "error" ? t("失败") : run.status;
			return h("div", { className: "dpo-run", "data-dpo": "run" },
				h("div", { className: "dpo-run-status", "data-dpo": "run-status", "aria-live": "polite" },
					t("档位 ") + ((TIERS.find((x) => x.id === run.tier) || {}).label || run.tier) + " · " + label + (run.firstPaintMs !== null ? t(" · 首字 {ms}ms", { ms: run.firstPaintMs }) : ""),
					chips && chips.total ? h("span", { className: "dpo-tok-chip", "data-dpo": "token-total", title: t("本次优化总 token") }, "Σ " + chips.total + " tok") : null,
					run.contextTurns > 0 && run.status !== "error" ? h("span", { className: "dpo-tok-chip", "data-dpo": "ctx-turns", title: t("已把本会话最近 {n} 轮对话作为消歧背景提供给优化模型（可在模型菜单里关闭）", { n: run.contextTurns }) }, t("背景 {n} 轮", { n: run.contextTurns })) : null,
					live && !chips ? h("span", { className: "dpo-tok-chip dpo-tok-live", "data-dpo": "token-wait", title: t("等待 provider 上报用量") }, "tok …") : null,
				),
				h("div", { className: "dpo-pane", "data-dpo": "pane-reasoning", "data-kind": "reasoning", "data-live": String(live && !run.reasoning), "aria-busy": String(Boolean(live && !run.reasoning)) },
					h("div", { className: "dpo-pane-title" },
						...
						chips && chips.reasoning
							? h("span", { className: "dpo-tok-chip", "data-dpo": "token-reasoning", title: t("本次思考消耗 token（provider 上报）") }, chips.reasoning + " tok")
							: (!live && run.status !== "error" ? h("span", { className: "dpo-tok-chip dpo-tok-muted", "data-dpo": "token-reasoning-none", title: t("该 provider 未上报思考 token") }, "— tok") : null),
						run.reasoning ? h("span", { className: "dpo-tok-chip dpo-tok-muted", title: t("思考字数") }, t("{n} 字", { n: run.reasoning.length })) : null,
					),
				...
				h("div", { className: "dpo-pane", "data-dpo": "pane-text", "data-kind": "text", "data-live": String(live && Boolean(run.reasoning)), "aria-busy": String(Boolean(live && Boolean(run.reasoning))) },
					h("div", { className: "dpo-pane-title" },
						t("产出"),
						chips && chips.output ? h("span", { className: "dpo-tok-chip", "data-dpo": "token-output", title: t("产出的输出 token") }, chips.output + " tok") : null,
						run.text ? h("span", { className: "dpo-tok-chip dpo-tok-muted", title: t("产出字数") }, t("{n} 字", { n: run.text.length })) : null,
					),
					h("div", { className: "dpo-pane-body", ref: liveScroll }, run.text || emptyOf("产出")),
				),
				run.status === "error"
					? h("div", { className: "dpo-run-error", "data-dpo": "run-error" },
						h("span", { title: String(run.error || "") }, t("失败：") + humanizeError(run.error)),
					)
					: null,
			);
		}
		...
		function copyResult(btn) {
			...
			const flash = (next) => {
				...
				if (next === t("已复制")) btn.setAttribute("data-copied", "1"); else btn.removeAttribute("data-copied");
			...
			const legacy = () => {
				...
				} catch (e) { flash(t("复制失败")); }
			};
			try {
				if (navigator.clipboard && navigator.clipboard.writeText) {
					navigator.clipboard.writeText(text).then(() => flash(t("已复制")), legacy);
				} else legacy();
			} catch (e) { legacy(); }
		}
		function reviewPane() {
			...
			return h("div", { className: "dpo-review", "data-dpo": "review" },
				h("div", { className: "dpo-pane-title", "data-dpo": "review-hint" },
					t("以下内容将原样发送（可直接编辑） · {n} 字", { n: String(text || "").length }),
					h("button", { type: "button", className: "dpo-copy", "data-dpo": "copy-review",
						title: t("复制全部内容"), onClick: (e) => copyResult(e.currentTarget) }, t("复制"))),
				store.regenAsk === true
					? h("div", { className: "dpo-regen-ask", "data-dpo": "regen-ask" },
						h("div", { className: "dpo-pane-title" }, t("重新生成 · 给个方向（可留空）")),
						h("input", {
							className: "dpo-regen-input", "data-dpo": "regen-input", type: "text", "aria-label": t("重新生成的方向（可留空）"),
							placeholder: t("例如：更短、保留技术细节、强调验收标准…"),
						...
						h("div", { className: "dpo-hint-quiet" }, t("回车即可重跑 · 留空则随机换一次方向")),
					)
					: null,
				h("textarea", {
					className: "dpo-review-text",
					"data-dpo": "review-text", "aria-label": t("优化后的内容（可直接编辑）"),
					value: text,
					spellCheck: false,
		...
		function overlayFooter() {
			...
			if (store.regenAsk === true) {
				return h("div", { className: "dpo-overlay-actions", "data-dpo": "foot-regen" },
					h("button", { type: "button", className: "dpo-btn primary", "data-dpo": "regen-go", onClick: submitRegen }, t("按此方向重跑")),
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "regen-cancel", onClick: () => { store.regenAsk = false; emit(); window.setTimeout(() => { try { const b = document.querySelector('[data-dpo="regen"]'); if (b && b.focus) b.focus({ preventScroll: true }); } catch (e) { /* noop */ } }, 80); } }, t("取消")),
				);
			}
			if (done && store.permission === "review") {
				return h("div", { className: "dpo-overlay-actions", "data-dpo": "foot-review" },
					h("button", { type: "button", className: "dpo-btn primary", "data-dpo": "confirm", onClick: () => { beacon("confirm-click", {}); confirmSubmit(); } }, t("确认提交")),
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "release", title: t("按原文直接发出，不优化"), onClick: releaseOriginal }, t("放行本条")),
					h("button", {
						...
						onClick: () => {
							...
							emit();
							window.setTimeout(() => { try { const el = document.querySelector('[data-dpo="regen-ask"]'); if (el && el.scrollIntoView) el.scrollIntoView({ block: "nearest" }); const inp = document.querySelector(".dpo-regen-input"); if (inp && inp.focus) inp.focus({ preventScroll: true }); } catch (e) { /* noop */ } }, 80);
						},
					}, t("重新生成")),
				);
			}
			if (err) {
				return h("div", { className: "dpo-overlay-actions", "data-dpo": "foot-error" },
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "retry", onClick: () => startRun(run.request, run.tier, false) }, t("重试")),
					h("button", { type: "button", className: "dpo-btn", "data-dpo": "reset-model", onClick: () => { store.modelSel = null; persistState({ tier: store.tier, permission: store.permission, model: null }); startRun(run.request, run.tier, false); } }, t("默认模型重试")),
					h("button", { type: "button", className: "dpo-btn primary", "data-dpo": "release", title: t("按原文直接发出，不优化"), onClick: releaseOriginal }, t("放行本条")),
				);
			}
			// 运行中 / 无运行 / 自动档：始终给一个"放行原文"的出口
			return h("div", { className: "dpo-overlay-actions", "data-dpo": "foot-idle" },
				h("button", { type: "button", className: "dpo-btn", "data-dpo": "rollback-here", onClick: () => { store.rollbackConfirm = true; emit(); window.setTimeout(() => { try { const el = document.querySelector('[data-dpo="rollback-confirm"]'); if (el && el.scrollIntoView) el.scrollIntoView({ block: "nearest" }); } catch (e) { /* noop */ } }, 80); } }, t("‹ 回退")),
				h("button", { type: "button", className: "dpo-btn primary", "data-dpo": "release", title: t("按原文直接发出，不优化"), onClick: releaseOriginal }, t("放行本条")),
			);
		}
		...
		function OverlayFallback(props) {
			return h("div", {
				...
				h("div", { className: "dpo-overlay-head", "data-dpo": "drag-handle" },
					h("button", { type: "button", className: "dpo-x", "data-dpo": "rollback", onClick: () => { store.rollbackConfirm = true; emit(); } }, t("‹ 回退")),
					h("span", null, t("提示词优化 · 渲染降级")),
					h("span", { className: "dpo-head-hint" }, t("已记录错误")),
				),
				h("div", { className: "dpo-overlay-scroll" },
					store.rollbackConfirm === true
						? h("div", { className: "dpo-overlay-src", "data-dpo": "rollback-confirm" },
							h("span", null, t("确定回退？将停止优化、关闭浮层，且不发送任何消息。")),
							h("button", { type: "button", className: "dpo-btn danger", "data-dpo": "rollback-yes", onClick: () => { beacon("rollback-yes", {}); rollbackYes(); } }, t("确定回退")),
							h("button", { type: "button", className: "dpo-btn", "data-dpo": "rollback-no", onClick: () => { store.rollbackConfirm = false; emit(); } }, t("取消回退")),
						)
						: null,
					h("div", { className: "dpo-overlay-body" }, t("浮层渲染出错，已降级为最小面板（优化仍在后台进行）。错误：") + String(props && props.err || t("未知"))),
		...
		function Overlay() {
			...
			// 焦点兜底：浮窗内任何操作若把当前按下的按钮卸载掉（按此方向重跑 / 重试 / 确认回退 …），
			// 焦点会掉到 <body> —— 此时把焦点收到浮窗容器上（已加 tabIndex=-1），用户按 Tab 就能
			// 在面板内继续走，而不是从页面开头重新数 79 个元素。只在依赖变化后检查一次，不持续抢焦点。
			React.useEffect(() => {
				if (!open) return undefined;
				const t = window.setTimeout(() => {
					try {
						const a = document.activeElement;
						if (a === document.body || a === null) {
			...
			}, [open, store.run ? store.run.status : null, store.regenAsk, store.rollbackConfirm]);
			...
			return h("div", {
				...
				"data-dpo": "overlay", role: "dialog", tabIndex: -1, "aria-label": t("提示词优化浮窗"),
				...
				h("div", { className: "dpo-overlay-head", "data-dpo": "drag-handle", onPointerDown, title: t("按住拖动（右下角可改大小）") },
					h("button", { type: "button", className: "dpo-x", "data-dpo": "rollback", title: t("回退：停掉本次优化并保留输入框原文"), onClick: () => { beacon("rollback-click", { confirm: store.rollbackConfirm === true }); store.rollbackConfirm = true; emit(); } }, t("‹ 回退")),
					h("span", null, t("提示词优化 · 已拦截")),
					h("span", { className: "dpo-head-hint" }, t("↘ 可改大小")),
				),
				store.overlay.blocked === true
					? h("div", { className: "dpo-blocked", "data-dpo": "blocked", role: "status" },
						store.overlay.src === "dup"
							? t("优化进行中 · 你的输入已保留在输入框，可稍候或点「回退」")
							: t("上一次的结果还没确认 · 你的输入已保留在输入框，请先「确认提交」或「回退」"))
					: null,
				h("div", { className: "dpo-overlay-scroll" },
					h("div", { className: "dpo-overlay-src" },
					t("发送：") + (PERMISSIONS.find((x) => x.id === store.permission) || {}).label + (store.intercepts.length > 0 ? t(" · {n} 已拦截", { n: store.intercepts.length }) : ""),
					(() => {
						// v130e：极端档的查证轨迹排在滚动区末尾 —— 实测浮窗 400x320、滚动区可视高仅 216px、内容 646px，
						// 轨迹面板需再滚 372px 才可见，等于"看不见"。极端档的核心价值就是看得见它查了什么，
						// 故在元信息行给一个跳转入口。矮视口（max-height:470px）本就 display:none 掉整行，自洽。
						const trc = store.run && store.run.trace ? store.run.trace : store.trace;
						const n = trc ? (Array.isArray(trc.rows) ? trc.rows.length : ((trc.normal || []).length + (trc.capped || []).length)) : 0;
						if (!store.run || store.run.tier !== "extreme" || n <= 0) return null;
						return h("button", { type: "button", className: "dpo-jump-trace", "data-dpo": "jump-trace", title: t("跳到查证轨迹"), onClick: () => {
							const el = document.querySelector('[data-dpo="trace"]');
					...
					store.rollbackConfirm === true
						? h("div", { className: "dpo-overlay-src", "data-dpo": "rollback-confirm" },
							h("span", null, t("确定回退？将停止优化、关闭浮层，且不发送任何消息。")),
							h("button", { type: "button", className: "dpo-btn danger", "data-dpo": "rollback-yes", onClick: () => { beacon("rollback-yes", {}); rollbackYes(); } }, t("确定回退")),
							h("button", { type: "button", className: "dpo-btn", "data-dpo": "rollback-no", onClick: () => { beacon("rollback-no", {}); store.rollbackConfirm = false; emit(); } }, t("取消回退")),
						)
						: null,
					runPanes(),
					reviewPane(),
					o.text ? h("div", { className: "dpo-orig", "data-dpo": "orig" }, h("div", { className: "dpo-pane-title" }, t("你的原文")), h("div", { className: "dpo-overlay-body" }, o.text)) : null,
					traceRows(),
				),
				...
				h("div", {
					className: "dpo-size-grip", "data-dpo": "resize", role: "separator", "aria-label": t("拖动改大小（记住）"), title: t("拖动改大小（记住）"),
		...
		exports.apply = function apply(ctx) {
			...
			/* v144：首屏读一次 DSH 的语言；若与默认值不同，延后一拍再 emit（那时 React 已挂载订阅）。 */
			if (syncLocale()) { try { window.setTimeout(() => emit(), 0); } catch (e) { /* noop */ } }
			if (localeService && typeof localeService.subscribe === "function") {
				let lastLabelsKey = null;
				own(() => localeService.subscribe(() => {
					if (syncLocale()) emit();   // v144：DSH 切语言时重渲染（订阅只在 active 变化时触发）
					const next = loadSendLabels();
					const key = next.join("\u0001");
					if (key === lastLabelsKey) return;   // v139：值未变则不上报（实测一次事件可触发约 40 次通知，均为同一值）
					lastLabelsKey = key;
					beacon("labels", { labels: next });
				}), NS + ": locale watch");
			}
			...
			fetch(API + "/state", { cache: "no-store" }).then((r) => r.json()).then((d) => {
				const st = d && d.ok ? d.state : null;
				// v154：宿主端设置文档写入失败（如孤儿写锁）时要让用户看得见、可处理
				if (d && typeof d.settings === "string") store.settingsSync = d.settings.indexOf("mirror-error") >= 0 ? "error" : "ok";
				if (!st) return;
				...
				// v120：会话背景开关是隐私偏好、与档位无关，必须放在下面的 touched 短路之前恢复
				if (typeof st.useContext === "boolean" && store.useContextTouched !== true) store.useContext = st.useContext;
				// v141：遥测开关同为偏好，必须放在下面的 touched 短路之前恢复
				// v154：加 touched 守卫 —— 用户刚点完开关时，迟到的 /state 响应不能把它改回去
				if (typeof st.debugEvidence === "boolean" && store.debugEvidenceTouched !== true) store.debugEvidence = st.debugEvidence;
				// v153：模型选择同为「偏好」而非档位值，必须放在下面的 touched 短路之前 ——
				// 否则用户手快改档位时它不会被恢复，紧接着的一次 persistState 就会把已落盘的模型清成 null。
			...
			own(() => {
				...
				style.textContent = [
					...
					"/* v120：会话背景开关（隐私相关，可关闭） */",
					".dpo-pop-priv{position:sticky;bottom:0;z-index:1;background:var(--dsw-alias-bg-layer-1,#fff);display:flex;align-items:center;gap:8px;padding:8px 12px 10px;border-top:.5px solid var(--dsw-alias-border-l2);flex-wrap:wrap}",
					".dpo-switch{display:inline-flex;align-items:center;gap:8px;background:transparent;border:0;padding:0;cursor:pointer;font:inherit;color:inherit}",
					".dpo-switch:focus-visible{outline:2px solid var(--dsw-alias-label-primary);outline-offset:2px;border-radius:6px}",
					".dpo-switch-track{position:relative;width:32px;height:18px;border-radius:9px;background:var(--dsw-alias-interactive-bg-hover);transition:background .18s cubic-bezier(.4,0,.2,1);flex:0 0 auto}",
					".dpo-switch-knob{position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:50%;background:var(--dsw-alias-label-secondary);box-shadow:0 1px 2px rgba(0,0,0,.25);transition:transform .18s cubic-bezier(.4,0,.2,1),background .18s cubic-bezier(.4,0,.2,1)}",
					".dpo-switch[data-on=\"true\"] .dpo-switch-track{background:var(--dsw-alias-label-primary)}",
					".dpo-switch[data-on=\"true\"] .dpo-switch-knob{transform:translateX(14px);background:var(--dsw-alias-bg-base)}",
					".dpo-switch-label{font-size:13px;font-weight:500;color:var(--dsw-alias-label-primary)}",
					".dpo-pop-priv-hint{font-size:11px;line-height:1.5;color:var(--dsw-alias-label-caption,var(--dsw-alias-label-tertiary));flex:1 1 100%}",
					".dpo-pop-priv-hint + .dpo-switch{margin-top:6px}",
					...
					"@media (max-width:1240px){.dpo-slider-track{flex-basis:88px;min-width:76px}.dpo-model{max-width:112px}.dpo-slider-value{font-size:12px}.dpo-controls{gap:9px}}",
										".dpo-notice{flex:0 0 auto;font-size:11px;padding:2px 8px;border-radius:10px;color:var(--dsw-alias-state-business-primary,#4a9eff);white-space:nowrap}",
					".dpo-count{flex:0 0 auto;font-size:10px;padding:1px 6px;border-radius:8px;background:var(--dpo-acc-12);color:var(--dsw-alias-state-business-primary,#4a9eff)}",
					...
					".dpo-btn.danger{flex:1;border:1px solid #d9534f;background:#d9534f;color:#fff;cursor:pointer}",
					".dpo-btn.primary{flex:1;border:1px solid var(--dsw-alias-state-business-primary,#4a9eff);background:var(--dsw-alias-state-business-primary,#4a9eff);color:#fff;cursor:pointer}",
					".dpo-run-error{display:flex;align-items:center;gap:8px;font-size:11px;word-break:break-all}",
					...
					".dpo-controls,.dpo-pop,.dpo-overlay,.dpo-notice-fixed{",
					...
					".dpo-slider-fill{top:7px;height:4px;border-radius:999px;background:linear-gradient(90deg,var(--dpo-acc),var(--dpo-acc-2));box-shadow:0 0 10px var(--dpo-acc-40);transition:width .18s var(--dpo-ease)}",
					".dpo-slider-thumb{top:2px;width:14px;height:14px;margin-left:-7px;background:radial-gradient(circle at 35% 30%,#fff,var(--dpo-acc-2) 45%,var(--dpo-acc));box-shadow:0 0 0 2px var(--dpo-surface),0 0 0 3px var(--dpo-acc-22),0 2px 6px rgba(0,0,0,.45);transition:left .18s var(--dpo-ease),transform .12s ease,box-shadow .18s var(--dpo-ease)}",
					".dpo-slider:hover .dpo-slider-thumb{box-shadow:0 0 0 2px var(--dpo-surface),0 0 0 4px var(--dpo-acc-22),0 3px 10px rgba(0,0,0,.5)}",
					".dpo-slider[data-disabled=\"true\"] .dpo-slider-thumb{animation:none}",
					".dpo-stop{position:relative}",
					".dpo-stop::after{content:\"\";position:absolute;left:50%;top:5px;width:4px;height:4px;margin-left:-2px;border-radius:50%;background:var(--dsw-alias-label-caption,#777);opacity:.5;transition:transform .12s ease,background .18s var(--dpo-ease),opacity .18s var(--dpo-ease)}",
					...
					".dpo-slider-value{font-weight:600;letter-spacing:.2px;padding:1px 7px;border-radius:999px;background:var(--dpo-acc-12);border:1px solid transparent;transition:background .18s var(--dpo-ease),color .18s var(--dpo-ease)}",
					".dpo-slider[data-disabled=\"true\"] .dpo-slider-value{background:transparent;color:var(--dsw-alias-label-caption,#777)}",
					".dpo-slider-value{animation:dpo-tick .32s var(--dpo-ease)}",
					/* 控件行：模型胶囊 */
					".dpo-model{height:26px;padding:0 10px;border-radius:999px;border:1px solid var(--dpo-line);background:linear-gradient(180deg,color-mix(in srgb,var(--dsw-alias-label-primary,#fff) 5%,transparent),transparent);box-shadow:var(--dpo-shadow-1);transition:none}",
					".dpo-model:hover{border-color:var(--dpo-acc);color:var(--dpo-acc);box-shadow:0 2px 10px var(--dpo-acc-22),var(--dpo-shadow-1)}",
					...
					".dpo-model-caret{flex:0 0 auto;font-size:9px;opacity:.6;transition:none}",
					...
					".dpo-notice{animation:dpo-notice-in .26s var(--dpo-ease);background:var(--dpo-acc-12);border:1px solid var(--dpo-acc-22);box-shadow:0 2px 12px var(--dpo-acc-12);font-weight:500}",
					/* 模型弹层 */
					".dpo-pop{width:272px;padding:8px;border-radius:14px;border:1px solid color-mix(in srgb,var(--dpo-line) 80%,transparent);box-shadow:var(--dpo-shadow-2);transform-origin:bottom left;scrollbar-gutter:stable}",
					...
					".dpo-pop-item{position:relative;display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:9px;font-size:12px;transition:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
					...
					".dpo-overlay-scroll{animation:dpo-ov-rise .3s var(--dpo-ease)}",
					...
					".dpo-overlay-head::before{content:\"\";flex:0 0 auto;width:7px;height:7px;margin-right:8px;border-radius:50%;background:var(--dsw-alias-label-caption,#777);transition:background .18s var(--dpo-ease)}",
					".dpo-overlay[data-state=\"running\"] .dpo-overlay-head::before{background:var(--dpo-acc);animation:dpo-pulse 1.6s ease-out infinite}",
					".dpo-x{display:inline-flex;align-items:center;gap:4px;height:22px;padding:0 9px;border-radius:999px;border:1px solid transparent;background:transparent;color:var(--dsw-alias-label-secondary,#bbb);font-size:11px;cursor:pointer;transition:background .18s var(--dpo-ease),color .18s var(--dpo-ease),border-color .18s var(--dpo-ease)}",
					...
					".dpo-pane{border-radius:12px;padding:8px 10px;border-color:color-mix(in srgb,var(--dpo-line) 80%,transparent);background:linear-gradient(180deg,color-mix(in srgb,var(--dsw-alias-label-primary,#fff) 3%,transparent),transparent);box-shadow:inset 0 1px 0 var(--dpo-hi);transition:border-color .18s var(--dpo-ease),box-shadow .18s var(--dpo-ease)}",
					...
					".dpo-review-text{border-radius:12px;padding:9px 11px;line-height:1.6;background:color-mix(in srgb,var(--dsw-alias-bg-layer-1,#141414) 92%,transparent);box-shadow:inset 0 1px 2px rgba(0,0,0,.35);transition:border-color .18s var(--dpo-ease),box-shadow .18s var(--dpo-ease)}",
					...
					".dpo-regen-ask{border-radius:12px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-1,#141414) 70%,transparent);box-shadow:inset 0 1px 0 var(--dpo-hi);animation:dpo-notice-in .24s var(--dpo-ease)}",
					".dpo-regen-input{border-radius:10px;height:28px;transition:border-color .18s var(--dpo-ease),box-shadow .18s var(--dpo-ease)}",
					...
					".dpo-btn{height:28px;border-radius:10px;font-weight:500;letter-spacing:.2px;transition:box-shadow .18s var(--dpo-ease),background .18s var(--dpo-ease),border-color .18s var(--dpo-ease),color .18s var(--dpo-ease)}",
					".dpo-btn:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);box-shadow:none}",
					...
					".dpo-run-error{border-radius:10px;padding:7px 9px;border:1px solid transparent}",
					".dpo-trace{border-top:1px solid color-mix(in srgb,var(--dpo-line) 60%,transparent)}",
					".dpo-trace-row{padding:3px 12px;border-radius:8px;transition:background .18s var(--dpo-ease)}",
					...
					".dpo-size-grip{width:16px;height:16px;right:3px;bottom:3px;border:none;border-radius:5px;opacity:.7;background:linear-gradient(135deg,transparent 42%,var(--dsw-alias-label-caption,#888) 42%,var(--dsw-alias-label-caption,#888) 52%,transparent 52%,transparent 62%,var(--dsw-alias-label-caption,#888) 62%,var(--dsw-alias-label-caption,#888) 72%,transparent 72%);transition:opacity .18s var(--dpo-ease),background .18s var(--dpo-ease)}",
					".dpo-size-grip:hover{opacity:1;background:linear-gradient(135deg,transparent 42%,var(--dpo-acc) 42%,var(--dpo-acc) 52%,transparent 52%,transparent 62%,var(--dpo-acc) 62%,var(--dpo-acc) 72%,transparent 72%)}",
					/* 降级面板也要好看 */
					/* 无障碍：尊重系统"减少动态效果" */
					"@media (prefers-reduced-motion:reduce){.dpo-controls *,.dpo-pop *,.dpo-overlay *,.dpo-notice-fixed{animation:none!important;transition:none!important}}",

					/* ══════════ v52 舒适层：更长的滑块 / 更疏朗的留白 / 更大的字号 ══════════ */
					".dpo-controls,.dpo-pop,.dpo-overlay,.dpo-notice-fixed{--dpo-fs-1:12px;--dpo-fs-2:13px;--dpo-fs-3:11.5px}",
					".dpo-controls{gap:10px}",
					...
					".dpo-pop-item:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);padding-left:12px;box-shadow:none}",
					...
					".dpo-help-v{flex:1 1 auto;color:var(--dsw-alias-label-secondary,#c9c9c9)}",
										/* v140c · 用 aria-disabled 而不是 disabled：busy 期间 React 把 button 设为 disabled 会让
   浏览器立刻把焦点从该元素移走（实测 activeElement 掉到 BODY），响应回来也不会自动回焦。
   aria-disabled 不脱离焦点树，配合 clearLogs() 开头的早退守卫同样能防重入。
   （v140d 补记：这条规则和 .dpo-clear-logs 其它规则一度全是死代码 —— JSX 里 className 忘了带
    dpo-clear-logs，只写了 dpo-btn，元素 matches(sel) 为 false。判定规则是否生效必须用 el.matches() 自证。） */
".dpo-clear-logs[aria-disabled=\"true\"]{opacity:.55;cursor:default;pointer-events:none}",
					".dpo-clear-logs:focus-visible{outline:2px solid var(--dsw-alias-label-primary);outline-offset:2px}",
					...
					"  transition:left .18s var(--dpo-ease),transform .12s ease,box-shadow .18s var(--dpo-ease);",
					"}",
					".dpo-controls .dpo-slider:hover .dpo-slider-thumb{box-shadow:0 1px 3px rgba(0,0,0,.24)}",
					"/* ══════════ v65 · 帮助弹层视口溢出修复 ══════════",
					...
					"   ★★ 第 48 轮实测更正（原注释「pop 的 90 是相对整页的」不成立）：",
					"     第 48 轮把 .dpo-pop 与 .dpo-overlay 的祖先链逐层 dump 后确认 ——",
					"       · .dpo-pop 的有效层级 = 7，不是 90：它唯一的 stacking-context 祖先是 DSH 的",
					"         .wSkVaW_composerSeat{position:sticky;z-index:7}（中间 .dpo-controls/.uV2eYG_*/.wSkVaW_composerStack",
					"         全部 z-index:auto + transform:none + opacity:1 + contain:none，不创建 context）；",
					"       · .dpo-overlay 的有效层级 = 20：被 .pI_x6G_overlayLayer{position:absolute;z-index:20} 锁定。",
					"     20 > 7 ⇒ 二者同时打开时**浮窗永远盖住弹层**，把 .dpo-pop 的 z-index 调到 1000 也没用。",
					"     硬证据：done 态下 elementFromPoint(769,314) 命中 DIV.dpo-review（浮窗）而非 .dpo-pop-item；",
					"     弹层 rect[589,244,368,270] 与浮窗 rect[640,48,400,320] 在 y293-368 重叠 ⇒",
					"     「关闭」档(y293-334)整行不可点、「普通」档(y334-375)只剩 7px 露出。",
					"     修法（留待下一轮）：portal 到 document.body —— .dpo-pop 自带全套 --dpo-*（见 :2714 的",
					"     .dpo-controls,.dpo-pop,.dpo-overlay{--dpo-acc:…}）与 --dpo-fs-*（见 :2827/:3463），portal 后样式自足。 ── */",
					".dpo-overlay{z-index:80}",
					"/* ── 34) pane 圆点去硬编码紫：早期版本给产出 pane 写死 background:#c08cff（v131 已删除该规则），",
					"   在 DSH 的单色体系里很突兀。改为「中性 = 静态 / 蓝色 = 正在流式」——",
					"   颜色只表达活跃度、不再表达种类；思考与产出本来就由标题文字区分。",
					"   实测两者 ::before 相同：浅色 rgb(173,178,184) / 暗色 rgb(129,133,140)（均 = label-caption）。 ── */",
					...
					"/* v130e 极端档查证轨迹跳转入口（元信息行内联，零布局影响：不用 flex，仅 margin-left） */",
					".dpo-jump-trace{margin-left:8px;font:inherit;font-size:11px;font-weight:500;color:var(--dpo-acc);background:var(--dpo-acc-12);border:0;border-radius:999px;padding:1px 8px;cursor:pointer;transition:none}",
					".dpo-jump-trace:hover{background:var(--dpo-acc-22)}",
					"/* ══════════ v72 · 档位 / 发送：滑块 → 下拉（产品层） ══════════ */",
					...
					".dpo-pop-chip-quiet{margin-left:auto;flex:0 0 auto;font-size:11px;color:var(--dsw-alias-label-tertiary,#8a8a8a);letter-spacing:0;white-space:nowrap}",
					...
					"   grid 布局下 justify-content 作用于列轨道，1fr 1fr 已占满，故本行对它无副作用。 */",
										"/* v140 · 底栏贴底常驻：帮助面板实测 h504 / scrollHeight1129，",
					"   「知道了」原本落在滚动区外（y1029），用户看不到关闭按钮；",
					"   清空日志入口同理 === 操作区必须常驻可见。背景取 --dsw-specific-menu（v128 已对齐 DSH 菜单底色）。 */",
					".dpo-pop-foot{position:sticky;bottom:0;z-index:2;background:var(--dsw-specific-menu,var(--dsw-alias-bg-layer-1,#fff));padding-bottom:14px}",
					...
					".dpo-overlay{border-radius:18px;outline:none}",
					".dpo-pop{background:var(--dsw-specific-menu,var(--dsw-alias-bg-layer-1,#fff))}",
					".dpo-pop .dpo-pop-priv{background:var(--dsw-specific-menu,var(--dsw-alias-bg-layer-1,#fff))}",
					".dpo-pop,.dpo-overlay{--dsw-elevation-prominent:0 0 0 .5px var(--dsw-alias-border-l1),0 3px 8px 0 #0000000a,0 0 20px 0 #0000000d}",
					".dpo-pop,.dpo-overlay{--dsw-elevation-stroke-color:var(--dsw-alias-border-l1)}",
					"/* 77-4 菜单项几何向 DSH 靠。",
					...
					".dpo-copy{margin-left:auto;flex:0 0 auto;font:inherit;font-size:11px;line-height:1;padding:4px 10px;border-radius:999px;border:0;background:transparent;color:var(--dsw-alias-label-tertiary,#81858c);cursor:pointer;white-space:nowrap;transition:background .18s var(--dpo-ease),color .18s var(--dpo-ease)}",
					...
					/* v134：toast 已是 body 下的 portal 节点。z-index 1090 —— 盖过浮窗所在层（DSH .pI_x6G_overlayLayer=20）
					   与自家弹层 .dpo-pop(90)，但低于 DSH 原生菜单 ._portal_1nxmc_44(1100)，不抢原生菜单。
					   pointer-events:none：纯展示反馈，浮在浮窗上时不该抢走点击。 */
					".dpo-notice-fixed{position:fixed;z-index:1090;max-width:min(440px,72vw);white-space:normal;text-wrap:pretty;pointer-events:none}",
					".dpo-sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);clip-path:inset(50%);white-space:nowrap;border:0}",
					"/* ══════════ v110 · 跟随 DSH 内容字号 ══════════ */",
					...
					".dpo-pop,.dpo-overlay,.dpo-notice-fixed{--dpo-fs-1:calc(12px + var(--dsh-content-font-delta,0px));--dpo-fs-2:calc(13px + var(--dsh-content-font-delta,0px));--dpo-fs-3:calc(11.5px + var(--dsh-content-font-delta,0px))}",
					...
					".dpo-pop{max-width:calc(100vw - 16px)}",
									"/* ══════════ v115 · 窄屏保留胶囊标签 ══════════ */",
					"/* 作者在 max-width:1080px 隐藏 .dpo-model-k；但 v69 把标签从「权限」改成「发送」",
					"   正是为了与 DSH 原生「完全权限」区分（窄屏下两者相邻），标签被隐藏会让该区分失效。",
					"   实测（984x510）：恢复标签后路由行 246→306px，无裁剪，composer 行仍有余量。",
					"   极窄（<900px）时隐藏，优先保全内容。 */",
					"@media (max-width:1080px){.dpo-controls .dpo-model-k{display:inline}}",
					"@media (max-width:900px){.dpo-controls .dpo-model-k{display:none}}",
					"/* ══════════ v116 · 高视口 .dpo-trace 高度与放大后的行高对齐 ══════════ */",
					"/* 必须限定 min-height:641px —— 与 v93 的 @media (max-height:640px) 同特异性，",
					"   CSS 后者胜，无边界的规则会连带覆盖矮视口。 */",
					"@media (min-height:641px){.dpo-trace{max-height:min(200px,26vh)}}",
].join("\n");
			...
			own(() => {
				...
				const timer = window.setInterval(() => {
					...
					// v148b：先让已注册的 Controls 重渲染（零成本、无副作用）。同 id 重复注册必被 DSH 拒绝，
					// 故仅在连续多次无效时低频尝试重挂；重挂包 try/catch，确保下面的放弃逻辑不会被异常跳过
					//（此前每次重挂都抛 "already has an entry"，把 tries >= 8 的放弃路径整段跳过 ⇒ 启动期白抛十余条未捕获异常）。
					for (const fn of Array.prototype.slice.call(store.listeners)) { try { fn(); } catch (e) { /* noop */ } }
					if (tries % 3 === 0) {
						try {
							own(() => ctx.slots.inject("conversation.input.left", () => ctx.slots.register({
								name: "conversation.input.left", id: "prompt-optimizer", order: 20,
							}, Controls)), NS + ": composer controls (retry " + tries + ")");
						} catch (e) { /* 插槽已注册：忽略，靠 emit 与下一次 tick 继续 */ }
					}
					if (tries >= 8) {
			...
			// v148：定时器常驻 —— 每次开窗独立计数（最多重挂 6 次，关窗归零），不再在插件启动约 3 秒后永久放弃。
			own(() => {
				let heals = 0;
				const timer = window.setInterval(() => {
					// 诊断面（window.__DPO_HEAL__）：定时器存活与最近判据，供"弹窗无声消失"定位；极轻量，无副作用
					try {
						const h = window.__DPO_HEAL__ || (window.__DPO_HEAL__ = { ticks: 0, open: false, hasNode: false, heals: 0 });
						h.ticks += 1; h.open = store.overlay.open === true; h.hasNode = !!document.querySelector('[data-dpo="overlay"]'); h.heals = heals;
					} catch (e) { /* noop */ }
					if (!store.overlay.open) { heals = 0; return; }
					if (document.querySelector('[data-dpo="overlay"]')) {
						if (heals > 0) { beacon("overlay-healed", { tries: heals }); heals = 0; }
						return;
					}
					if (heals >= 6) return;
					heals += 1;
					beacon("remount-overlay", { tries: heals, runStatus: store.run ? store.run.status : null, tier: store.tier });
					// v148b：先让已注册的 OverlayHost 重渲染 —— 零成本、无副作用，覆盖绝大多数"没画出来"。
					for (const fn of Array.prototype.slice.call(store.listeners)) { try { fn(); } catch (e) { /* noop */ } }
					// 连续多次重渲染都无效，才怀疑插槽从未挂上并尝试重新注册；同 id 重复注册会被 DSH 拒绝
					//（"already has an entry with id …"），故按 3 次一档低频尝试，避免异常噪声淹没日志。
					if (heals % 3 === 0) {
						own(() => ctx.slots.inject("shell.overlay", () => ctx.slots.register({
							name: "shell.overlay", id: "prompt-optimizer-overlay", order: 60,
						}, OverlayHost)), NS + ": overlay retry " + heals);
					}
					if (heals >= 6) beacon("overlay-slot-missing", { tries: heals });
				}, 1500);
				return () => window.clearInterval(timer);
