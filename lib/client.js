...
const EN_DICT = {
	...
	"点击关闭：关闭后不再读取本会话的任何历史消息 | 会话背景：把本会话最近的对话作为消歧背景提供给优化模型": "Click to turn off: no history from this session will be read | Session context: provides the recent conversation in this session to the optimization model as disambiguation context",
...
function beacon(stage, data) {
		...
		function helpPopover() {
			...
			return h("div", {
				...
				h("div", { className: "dpo-help-meta", "data-dpo": "help-meta" },
					"上游 " + UPSTREAM.author + " " + UPSTREAM.version + "（" + UPSTREAM.date + "） · LessXi fork v" + PLUGIN_VERSION),
			);
		}
			...
			return h("div", {
				...
				h("div", { className: "dpo-pop-priv", "data-dpo": "ctx-row" },
					h("button", {
						...
						"aria-label": "会话背景：把本会话最近的对话作为消歧背景提供给优化模型",
		...
		function traceRows() {
			...
			const t = live || store.trace;
			if (!t) return null;
			const rows = live
				? (t.rows || []).map((x) => ({ ...x, phase: "查证" }))
				: [...(t.normal || []).map((x) => ({ ...x, phase: "查证" })), ...(t.capped || []).map((x) => ({ ...x, phase: "收尾" }))];
			if (rows.length === 0) {
		...
		function Overlay() {
			...
			return h("div", {
				...
				h("div", { className: "dpo-overlay-scroll" },
					...
					t("发送：") + (PERMISSIONS.find((x) => x.id === store.permission) || {}).label + (store.intercepts.length > 0 ? " · " + store.intercepts.length + " " + t("已拦截") : ""),
					(() => {
						...
						const t = store.run && store.run.trace ? store.run.trace : store.trace;
						const n = t ? (Array.isArray(t.rows) ? t.rows.length : ((t.normal || []).length + (t.capped || []).length)) : 0;
