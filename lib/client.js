window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function tierPopover() {
			...
			return h("div", {
				className: "dpo-pop", "data-dpo": "tier-pop",
				...
				h("div", { className: "dpo-pop-group" },
					TIERS.map((t) => h("button", {
						key: t.id, type: "button", className: "dpo-pop-item", "data-dpo": "tier-item",
		...
		function permPopover() {
			...
			return h("div", {
				className: "dpo-pop", "data-dpo": "perm-pop",
				style: { left: pos.x + "px", bottom: pos.bottom + "px", maxHeight: pos.maxH + "px" },
			},
				h("div", { className: "dpo-pop-head" }, "优化完成后"),
				h("div", { className: "dpo-pop-group" },
					PERMISSIONS.map((p) => h("button", {
						key: p.id, type: "button", className: "dpo-pop-item", "data-dpo": "perm-item",
		...
		function Controls(props) {
			...
			return h("div", { ref: nodeRef, className: "dpo-controls", "data-dpo": "controls" },
						...
						onClick: (e) => { const r = e.currentTarget.getBoundingClientRect(); store.tierPop = { x: Math.max(8, Math.min(r.left, window.innerWidth - 320)), bottom: Math.max(8, window.innerHeight - r.top + 6), maxH: Math.max(140, r.top - 16) }; const willOpen = !store.tierPopOpen; closePops(willOpen ? "tier" : null); emit(); },
					},
				...
				h("button", {
					...
					onClick: (e) => { if (off) return; const r = e.currentTarget.getBoundingClientRect(); store.permPop = { x: Math.max(8, Math.min(r.left, window.innerWidth - 320)), bottom: Math.max(8, window.innerHeight - r.top + 6), maxH: Math.max(140, r.top - 16) }; const willOpen = !store.permPopOpen; closePops(willOpen ? "perm" : null); emit(); },
				},
				...
				h("button", {
					...
					onClick: (e) => { const r = e.currentTarget.getBoundingClientRect(); store.modelPop = { x: Math.max(8, Math.min(r.left, window.innerWidth - 320)), bottom: Math.max(8, window.innerHeight - r.top + 6), maxH: Math.max(140, r.top - 16) }; const willOpen = !store.modelPopOpen; closePops(willOpen ? "model" : null); emit(); },
				},
				...
				h("button", {
					...
					"data-open": String(store.helpOpen === true), "aria-haspopup": "menu", "aria-expanded": String(store.helpOpen === true),
				...
				store.overlay.blocked === true
					? h("div", { className: "dpo-blocked", "data-dpo": "blocked", role: "status" },
						store.overlay.src === "dup"
							? "优化进行中 · 你刚输入的内容已保留在输入框，可稍候或点「回退」"
							: "上一次的优化结果还等你确认 · 你刚输入的内容已保留在输入框，请先点「确认提交」或「回退」")
					: null,
				h("div", { className: "dpo-overlay-scroll" },
		...
		exports.apply = function apply(ctx) {
			...
			own(() => {
				...
				style.textContent = [
					...
					".dpo-blocked{padding:8px 16px;font-size:11.5px;line-height:1.5;color:var(--dsw-alias-state-business-primary,#4176e6);background:color-mix(in srgb,var(--dsw-alias-state-business-primary,#4176e6) 8%,transparent);border-bottom:1px solid color-mix(in srgb,var(--dsw-alias-state-business-primary,#4176e6) 20%,transparent)}",
].join("\n");
