		...
		function runPanes() {
			...
			return h("div", { className: "dpo-run", "data-dpo": "run" },
				h("div", { className: "dpo-run-status", "data-dpo": "run-status", "aria-live": "polite" },
					...
					run.contextTurns > 0 && run.status !== "error" ? h("span", { className: "dpo-tok-chip", "data-dpo": "ctx-turns", title: t("已把本会话最近 {n} 轮对话作为消歧背景提供给优化模型（可在模型菜单里关闭）", { n: run.contextTurns }) }, t("背景 {n} 轮", { n: run.contextTurns }) + + " 轮") : null,
		...
		function Overlay() {
			...
			return h("div", {
				...
				h("div", { className: "dpo-overlay-head", "data-dpo": "drag-handle", onPointerDown, title: t("按住拖动（右下角可改大小）") },
					h("button", { type: "button", className: "dpo-x", "data-dpo": "rollback", title: t("回退：停掉本次优化并保留输入框原文"), onClick: () => { beacon("rollback-click", { confirm: store.rollbackConfirm === true }); store.rollbackConfirm = true; emit(); } }, "‹ 回退"),
