window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function interceptAndOptimize(text) {
			...
			setOverlay({ open: true, text: body, fullText: body, src: "optimize", sessionId: store.viewSessionId });
		...
		function startRun(request, tier, forceError, opts) {
			...
			store.reviewText = null;
			store.regenAsk = false;
			setOverlay({ open: true, text: String(request || ""), fullText: String(request || ""), src: "run", sessionId: sid });
			emit();
