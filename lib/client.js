...
function beacon(stage, data) {
		...
		function helpPopover() {
			...
			return h("div", {
				...
				line("本地日志", "默认不写任何日志：平时打字、拦截、切换档位都不会落盘或发请求（仅未捕获错误与自愈失败会留证）。日志只记长度、计数与状态，不含你的输入原文；单个超过 2MB 自动轮转，仅保留最近两代，全部保存在本机、不会上传"),
			...
			return h("div", {
				...
				h("div", { className: "dpo-pop-priv", "data-dpo": "ctx-row" },
					h("button", {
						...
						onClick: () => { store.useContext = !(store.useContext !== false); store.useContextTouched = true; persistState({ useContext: store.useContext }); store.modelPopOpen = false; emit(); },
					},
	...
	window.setTimeout(() => {
		...
		exports.apply = function apply(ctx) {
			...
			fetch(API + "/state", { cache: "no-store" }).then((r) => r.json()).then((d) => {
				...
				if (typeof st.debugEvidence === "boolean") store.debugEvidence = st.debugEvidence;
