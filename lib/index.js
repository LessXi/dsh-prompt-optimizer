/**
 * @lessxi/dsh-prompt-optimizer — host 半（P0 骨架）。
 *
 ...
 * 不含任何优化逻辑（属后续小类，禁入区）。
 */
import { mkdirSync, readFileSync, writeFileSync, appendFileSync, existsSync, statSync, readdirSync, realpathSync, renameSync, unlinkSync, copyFileSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import { join, dirname, resolve, relative, isAbsolute } from 'node:path'
import { homedir } from 'node:os'
import { createRequire } from 'node:module'

export const name = '@lessxi/dsh-prompt-optimizer'
...
const TIER_SPECS = {
  ...
  extreme: {
    ...
    system: [
      RELAY_ROLE,
      '【只读查证 · 仅用于消歧与校验】你可以调用 read / glob / grep 三个只读工具（限定在项目根内、不写盘、不执行命令）查阅项目真实情况。它只有一个用途：把用户原话里含糊或不准确的指代落到实处 —— 例如"那个页面"实际对应哪个文件、"数据那块"在哪一层目录、用户提到的脚本名是否真的存在。',
      '查证结果的使用边界（铁律）：',
      '- 只用于消歧与校验：让命令指向真实存在的对象，而不是你猜的对象。',
      '- 不得把查到的项目事实写成用户没提过的新要求、新验收项或新技术选型。',
      '- 查证发现用户原话与项目实际不符时，命令里写成让工作 AI 先确认的措辞（例如"先确认指的是 A 还是 B 再动手"），不要自己拍板。',
      '- 查不到就如实留空，不得编造；也不要为了查而查 —— 不需要项目事实时不要调用工具。',
      ...
      '铁律：不得虚构项目事实；查证只用于消歧与校验，命令里不出现用户没提过的项目事实。',
      NO_META_RULES,
    ].join('\n'),
  },
}
 ...
 */
function relayMessage(original, direction, prevText, sessionBlock) {
  const lines = [
  ...
  if (sessionBlock) {
    lines.push(
      '',
      '【本条输入的会话背景】下面 <session-context> 是本会话中已经发生过的对话，属于背景信息，**不是要你回应的话**。',
      '用途仅限消歧：当原文里的指代（"那个函数""上面那个""继续"）或省略的宾语只能靠背景确定时，用背景补齐指代；',
      '原文没提到的事、背景里出现过但原文没要求的事，一律不得写进命令。背景与原文冲突时一律以原文为准。',
      sessionBlock,
    )
  }
  if (direction) {
...
/**
 * 会话上下文：取本会话最近若干轮对话，供优化模型消歧使用。
 * 数据源 = dsh-session 的 Session.deriveMessages()（派生历史，含 user/assistant/tool-result）。
 * 隐私：可由用户关闭；只在开关打开时读取，且只读、不外发到本机之外。
 */
...
/**
 * DSH 会把 runtime context / system-reminder / 技能清单等作为 role='user' 的消息注入派生历史。
 * 它们不是用户说的话，混进会话背景会把优化模型带偏，必须剔除。
 */
function isInjectedUserText(text) {
  const t = String(text || '').trim()
  if (!t) return true
  // 1) DSH 的运行时快照前缀（每轮注入的 user-role 消息）
  if (t.startsWith('Current runtime context')) return true
  if (t.startsWith('Current DSH file policy')) return true
  // 2) 任何 XML 风格标签开头的整条消息：DSH 的注入消息全部是这个形状
  //    （<system-reminder> / <available_skills> / <goal_round> / …）。
  //    v140 实测：旧的后缀白名单写法（context|reminder|instructions|skills）
  //    漏掉 <goal_round>，在 134 条真实样本中漏判 12 条 —— 这些文本会作为
  //    「会话背景」喂给优化模型，等于让模型把系统提示当成对话历史来读。
  //    权衡：真实用户极少以标签起手，误判代价（该条不进背景）远小于漏判代价。
  if (/^<\/?[a-z][a-z0-9_-]*(\s[^>]*)?>/i.test(t)) return true
  // 3) 系统事件播报：子代理/后台任务完成、上下文压缩回执、效率提示
  if (/^(?:Background|background)\s+(?:subagent|job)\b/.test(t)) return true
  if (/^Compressed\s+\d+\s+block/i.test(t)) return true
  if (/^This is an efficiency nudge/i.test(t)) return true
  return false
}

function collectSessionContext(ctx, sessionId, opts) {
  const maxTurns = Math.max(1, Math.min(20, Number((opts && opts.maxTurns) || CONTEXT_MAX_TURNS)))
  try {
    const sessions = ctx.get('sessions')
    if (!sessions) return { block: '', turns: 0, messages: 0, reason: 'no-sessions-service' }
    let session = null
    if (sessionId && typeof sessions.get === 'function') {
      try { session = sessions.get(sessionId) || null } catch { session = null }
    }
    // 已查明 get() 期望 SessionId 类型：客户端传来的字符串可能不匹配，用 list() 再按 id 精确补一次。
    // **绝不回退 list()[0]**：会话背景属于会话内容，认不出是哪个会话时宁可不用背景，
    // 也不能把别的会话的历史当成本会话的背景 —— 那既是跨会话读取，也会把优化带偏。
    // （v124 修：原先写的是 `|| list[0]`，实测假 sessionId 只拿到 empty-history 而不是
    //  session-not-found，说明这条路真的会被走到，只是当轮 list 首项恰好无历史。）
    if (!session && sessionId && typeof sessions.list === 'function') {
      const list = sessions.list() || []
      session = list.find((s) => String(s.id) === String(sessionId)) || null
    }
    if (!session) return { block: '', turns: 0, messages: 0, reason: 'session-not-found' }
    if (typeof session.deriveMessages !== 'function') return { block: '', turns: 0, messages: 0, reason: 'no-derive-messages' }
    const all = session.deriveMessages() || []
    // 只保留 user/assistant 的纯文本；ToolResultMessage 的 role 也是 'user'，但 content 非 text 块，已自然过滤
    const rows = []
    for (const msg of all) {
      ...
      if (!text) continue
      // 排除 DSH 注入的伪用户消息：它们的 role 也是 'user'，但不是"用户说过的话"，
      // 混进来会让优化模型把 runtime context / system-reminder 当成对话背景。
      if (role === 'user' && isInjectedUserText(text)) continue
      rows.push({ role, text })
    }
    // 兜底：若当前这条已被写入会话（消息先落库再拦截的情形），去掉末尾与之相同的那条
    const request = opts && opts.request ? String(opts.request).trim() : ''
    if (request && rows.length > 0) {
      const last = rows[rows.length - 1]
      if (last.role === 'user' && last.text.trim() === request) rows.pop()
    }
    // 从末尾往前取，凑满 maxTurns 个用户发言为止
    const picked = []
    let userSeen = 0
    ...
    picked.reverse()
    if (picked.length === 0) return { block: '', turns: 0, messages: 0, reason: 'empty-history' }
    const lines = []
    let chars = 0
    for (const row of picked) {
      const label = row.role === 'user' ? '用户' : '助手'
      const body = clipText(row.text, CONTEXT_MSG_CHARS)
      const line = label + '：' + body
      if (chars + line.length > CONTEXT_MAX_CHARS) break
      chars += line.length
      lines.push(line)
    }
    if (lines.length === 0) return { block: '', turns: 0, messages: 0, reason: 'over-budget' }
    const block = [
      '<session-context>',
      '以下是本会话中已经发生的对话（越靠后越新，最新一条是本次待整理输入之前的内容）。',
      ...lines,
      '</session-context>',
    ].join('\n')
    // 透明可查：只落盘「规模 + 指纹」，绝不落原文（会话内容比单条输入敏感）
    try {
      let fp = 5381
      for (let k = 0; k < block.length; k += 1) fp = ((fp << 5) + fp + block.charCodeAt(k)) >>> 0
      appendBounded(join(EVIDENCE, 'session-context.jsonl'), JSON.stringify({
        t: Date.now(), sessionId: sessionId || null, turns: userSeen, messages: lines.length,
        chars: block.length, fp: fp.toString(16), roles: picked.map((r) => r.role[0]).join(''),
      }) + '\n')
    } catch { /* evidence is best-effort */ }
    return { block, turns: userSeen, messages: lines.length, chars: block.length }
  } catch (err) {
    return { block: '', turns: 0, messages: 0, reason: 'error:' + String((err && err.message) || err) }
  }
}
...
// v138 健壮性加固（安全审计首次覆盖这一维度）：
// 宿主是单线程 Node。runToolLoop 的 LOOP_BUDGET_MS 靠 ac.abort() 只中止 LLM 流，
// 而 read/glob/grep 是同步函数 —— 同步代码执行期间 setTimeout 回调根本不会跑，
// 所以「灾难性回溯的正则」会把宿主进程永久卡死（DSH 无响应、只能重启）。
// 四道防线：pattern 长度 → 回溯形状检测 → 整体时间预算 → 单行送入上限。
// 预算取 8000ms 的依据（实测）：walkFiles 上限 400 文件 × LOOP_MAX_FILE_BYTES 200KB，
// 正常查证（如 "发送拦截|拦截发送|sendInterceptor"）在真实项目上会跑满 1.5s，
// 1500ms 会把它截断、导致结论缺证据。ReDoS 由防线 2 零成本拦住，故预算只需给足正常用量。
// 防线 4 仍保留 2000：源码正常行远小于 200，它挡的是 minified 单行（可达数十万字符）。
const LOOP_GREP_MAX_PATTERN = 300
const LOOP_GREP_MAX_LINE = 2000
const LOOP_GREP_BUDGET_MS = 8000
// 单条待优化文本的字符上限（超出会变成巨额 token 与超时；客户端与宿主端各自拦一道）
...
/** 灾难性回溯（ReDoS）形状：量词直接作用于内含量词的分组 → 指数级回溯。
 *  已知局限：不检测交替重复（(a|a)+），因为 (foo|bar)+ 这类常见安全写法会被误伤；
 *  也不试图判「(a+b)* 其实安全」这类语义豁免 —— 宁可让模型改写一次，也不冒卡死宿主进程的风险。 */
function riskyRegex(src) {
  return /\([^()]*[*+][^()]*\)\s*[*+{]/.test(String(src))
}

function toolGrep(root, args) {
  const pattern = String(args.pattern || '')
  // 防线 1：超长正则本身就是异常输入
  if (pattern.length > LOOP_GREP_MAX_PATTERN) {
    return '拒绝：正则过长（' + pattern.length + ' 字符，上限 ' + LOOP_GREP_MAX_PATTERN + '）'
  }
  // 防线 2：回溯形状 → 直接拒绝。这里绝不能"先跑跑看"：同步爆炸会卡死整个 DSH。
  if (riskyRegex(pattern)) {
    return '拒绝：正则含嵌套量词（如 (a+)+ / (a*)*），可能触发灾难性回溯并卡住 DSH；请改写为不嵌套的等价写法（例如 (a+)+ 改为 a+、(\\w+\\s*)+ 改为 [\\w\\s]+）后重试'
  }
  let rx = null
  try { rx = new RegExp(pattern) } catch (error) { return '正则无效：' + String(error) }
  ...
  for (const abs of files) {
    ...
    // 防线 3：整体时间预算（挡"每次回溯都不算大、但次数极多"的累积型慢正则）
    if (Date.now() - t0 > LOOP_GREP_BUDGET_MS) { budgetHit = true; break }
    let info = null
    ...
    for (let i = 0; i < lines.length && hits.length < 40; i += 1) {
      const line = lines[i]
      // 防线 4：限制送进正则的字符数（回溯规模的上界就是输入长度；minified 单行可达数十万字符）
      const probe = line.length > LOOP_GREP_MAX_LINE ? line.slice(0, LOOP_GREP_MAX_LINE) : line
      if (rx.test(probe)) hits.push(rel + ':' + (i + 1) + ': ' + line.trim().slice(0, 200))
    }
  }
  const body = hits.length === 0 ? '无匹配（pattern=' + pattern + '）' : '命中 ' + hits.length + ' 条：\n' + hits.join('\n')
  return budgetHit ? body + '\n…（查证耗时超过 ' + LOOP_GREP_BUDGET_MS + 'ms 预算，已提前结束）' : body
}
...
async function runToolLoop(ctx, opts) {
  ...
  // opts.userText 既可能是纯文本（自检路径），也可能是构造好的消息数组（relayMessage 的产物）
  const messages = Array.isArray(opts.userText) ? opts.userText : userMessageFor(opts.userText)
  let rounds = 0
  let converged = false
  let capped = false
  let output = ''
  ...
  for (let round = 1; round <= maxRounds + 1; round += 1) {
    ...
    if (opts.onReset) opts.onReset(round)
    const res = await streamWithTools(llm, selection, system, messages, useTools ? LOOP_TOOLS : undefined, TIER_SPECS.extreme.temperature, deadline - Date.now(), opts.externalSignal, opts.onDelta)
    rounds = round
    ...
    for (const call of res.calls) {
      ...
      trace.push(rec)
      if (opts.onTrace) opts.onTrace(trace.slice())
...
function loadPluginState() {
  ...
  return {
    tier: raw && TIER_IDS.has(raw.tier) ? raw.tier : 'basic',
    permission: raw && PERMISSION_IDS.has(raw.permission) ? raw.permission : 'review',
    ...
    // v120：会话背景开关，默认开启；只有显式存成 false 才算关闭
    useContext: !(raw && raw.useContext === false),
    // v139：调试取证开关，默认关（开启才落截断原文）
    debugEvidence: raw && raw.debugEvidence === true,
    revision: raw && typeof raw.revision === 'number' ? raw.revision : 0,
  }
}
let settingsScope = null
let settingsStatus = 'not-initialized'
let lastMirrorKey = null
// v153：上一次推给设置文档的「preference 投影」，用来识别并忽略自己造成的 watch 回声。
let lastMirrorFingerprint = null
/* v154：设置文档只承载「用户偏好」两项布尔。
   历史教训：v149 把内部运行时状态（tier/permission/model/ui）也注册成设置字段，等于制造第二份权威 ——
   任何探针/测试写入都会变成"用户设置"，实测残留曾把 permission 写成 auto（静默自动发送）、
   debugEvidence 写成 true（记录输入原文）。收窄后设置文档里只剩这两个开关，
   宿主内部状态仍只走 prompt-optimizer.json。 */
function settingsPrefs(state) {
  const o = state && typeof state === 'object' ? state : {}
  return {
    useContext: o.useContext !== false,
    debugEvidence: o.debugEvidence === true,
  }
}
function loadSettingsSchema() {
  const home = process.env.DSH_HOME || join(homedir(), '.dsh')
  const req = createRequire(join(home, 'profiles', 'web', 'package.json'))
  // dsh-settings 的注册契约是 @deepseek-ai/schemastery（见其 lib/types/index.d.ts 的 import type z）。
  // 注意 schemastery 没有 .optional()/.nullable()：可选或可空字段一律用 .required(false)。
  const mod = req('@deepseek-ai/schemastery')
  const z = mod && mod.default ? mod.default : mod
  if (!z || typeof z.object !== 'function') throw new Error('schemastery has no z.object')
  return z
}
function initSettingsNamespace(settingsCtx) {
  try {
    const z = loadSettingsSchema()
    const settings = settingsCtx && settingsCtx.settings ? settingsCtx.settings
      : (settingsCtx && typeof settingsCtx.get === 'function' ? settingsCtx.get('settings') : null)
    if (!settings || typeof settings.register !== 'function') { settingsStatus = 'no-settings-service'; return settingsStatus }
    // v154：只注册两项用户偏好（理由见 settingsPrefs 上方注释）。
    // description 用 { "": 中文, en: English } —— dsh-client-locale 的 mergeDesc 接受这种形状。
    // 注意：DSH 设置面板「插件」tab 只渲染通过 settings.plugin.item 贡献的卡片，
    // 本插件没有卡片，因此这两条 description 目前不会显示在面板里；
    // 控件放在插件自己的模型菜单底部（见 lib/client.js 的 diag-toggle）。
    const schema = z.object({
      useContext: z.boolean().default(true).description({
        '': '发送前把本会话最近的对话作为消歧背景（仅用于补全指代；关闭后不读取任何历史）',
        en: 'Sends the recent conversation in this session as disambiguation background (used only to resolve references; when off, no history is read)',
      }),
      debugEvidence: z.boolean().default(false).description({
        '': '把输入与结果的前几十字写进本机诊断日志，便于排查问题（日志不上传，超 2MB 自动轮转）',
        en: 'Writes the first few dozen characters of your input and results to the local diagnostic log for troubleshooting (never uploaded; rotates at 2 MB)',
      }),
    })
    settingsScope = settings.register('prompt-optimizer', schema)
    settingsStatus = 'registered'
    // 反向同步：用户在 DSH 设置面板改了就写回插件自己的状态（否则会出现「改了不生效」）。
    // 终止条件靠「与当前状态比较」而不是标志位：写回后两边一致，下一次回调 patch 为空即返回。
    try {
      if (settingsScope && typeof settingsScope.watch === 'function') {
        settingsScope.watch((next) => {
          try {
            if (!next || typeof next !== 'object') return
            // v153：这一事件若正是我们自己刚推上去的值，就不要再反写回来（否则两者会来回覆盖）。
            // v154：只对账设置文档真正承载的两项偏好；tier/permission 已不在 schema 里。
            const prefs = settingsPrefs(next)
            if (JSON.stringify(prefs) === lastMirrorFingerprint) return
            const cur = loadPluginState()
            const patch = {}
            if (prefs.useContext !== (cur.useContext !== false)) patch.useContext = prefs.useContext
            // v153：设置面板里的「诊断取证」原本无人转发 ⇒ 开关完全无效（值永远停在 false）
            if (prefs.debugEvidence !== (cur.debugEvidence === true)) patch.debugEvidence = prefs.debugEvidence
            if (Object.keys(patch).length === 0) return
            savePluginState(patch)
          } catch { /* best effort */ }
        })
      }
    } catch { /* watch 不可用不影响注册 */ }
  } catch (error) {
    settingsStatus = 'error: ' + String(error)
  }
  return settingsStatus
}
function mirrorToSettings(state) {
  try {
    if (!settingsScope || typeof settingsScope.update !== 'function') {
      // 只在「服务从未初始化」时降级；否则保留 no-settings-service / error: 这类更有信息量的诊断。
      if (settingsStatus === 'not-initialized') settingsStatus = 'mirror-unavailable'
      return
    }
    // v154：只镜像设置 schema 暴露的两项用户偏好，内部运行时状态不进设置文档。
    // 值未变则直接返回 —— 否则每次落盘都会 bump 设置 revision 并广播事件（同 v139 的 labels 去重）。
    // v153 教训（漏镜像）：设置文档会停在 schema 默认值，而 watch 又把那个陈旧默认值当权威写回 ——
    //   用户关闭「会话背景」后只要再换一次档位，开关就被静默改回开启（隐私选择被撤销）。
    // v154 教训（过度镜像）：把内部状态也写进设置文档，则任何探针写入都变成"用户设置"。
    const mirror = settingsPrefs(state)
    const key = JSON.stringify(mirror)
    if (key === lastMirrorKey) return
    lastMirrorKey = key
    lastMirrorFingerprint = key
    Promise.resolve(settingsScope.update(mirror)).then(
      () => { settingsStatus = 'registered+mirrored' },
      (error) => {
        // v154：写失败必须撤掉「已镜像」的账。实测：孤儿写锁（atomic-write 拒绝删除他人锁）
        // 导致 update 超时后，同值再也不会重试 —— 设置文档永久停在陈旧值，且无人知晓。
        // 清掉记账后，下一次落盘（任何一次改档位/几何/开关）就会自动补上。
        if (lastMirrorKey === key) { lastMirrorKey = null; lastMirrorFingerprint = null }
...
function savePluginState(patch) {
  ...
  const next = {
    tier: typeof patch.tier === 'string' && TIER_IDS.has(patch.tier) ? patch.tier : cur.tier,
    permission: typeof patch.permission === 'string' && PERMISSION_IDS.has(patch.permission) ? patch.permission : cur.permission,
    ...
    // v120：会话背景开关（隐私相关，全局偏好而非 per-session）——只有显式 false 才关闭
    useContext: patch.useContext === undefined ? (cur.useContext !== false) : patch.useContext === true,
    // v153：与上一行的 useContext 同构 —— 未显式携带时保留现值。
    // 否则每次拖动浮窗（persistUi 只发 ui）或换档位（persistState 不发它）都会把诊断取证静默关掉。
    debugEvidence: patch.debugEvidence === undefined ? cur.debugEvidence === true : patch.debugEvidence === true,
    revision: cur.revision + 1,
    updatedAt: Date.now(),
  }
  // v153：值没变就不落盘。除了省掉无谓的磁盘写，这还让任何「反向同步 ↔ 落盘」的来回
  // 在值不变时自然终止 —— 实测曾观察到 33 次/秒的空转写入（12 秒内 revision +400）。
  // 归一化必须覆盖**全部**持久化字段，否则会把真实变更误判成无变化（那是数据丢失）。
  const unchanged = (() => {
    try {
      const raw = readJson(STATE_FILE, null)
      ...
      const norm = (o) => JSON.stringify({
        ...
        model: o.model ? { provider: o.model.provider || null, model: o.model.model || null, name: o.model.name || null } : null,
        ui: o.ui ? { w: o.ui.w || null, h: o.ui.h || null, x: o.ui.x === undefined ? null : o.ui.x, y: o.ui.y === undefined ? null : o.ui.y } : null,
        perSession: Object.keys(o.perSession || {}).sort().map((k) => [k, (o.perSession[k] || {}).tier || null, (o.perSession[k] || {}).permission || null]),
        useContext: o.useContext !== false,
        debugEvidence: o.debugEvidence === true,
      })
      return norm(raw) === norm(next)
    } catch (e) { return false }
  })()
  if (unchanged) {
    next.revision = cur.revision
    next.skippedWrite = true
    ...
    try {
      ...
      next.saveError = String((error && error.message) || error)
    }
  }
  mirrorToSettings(next)
  return next
}
...
/* v92 · 运行记录的保留上限。liveRuns 原先只 set 从不 delete，
   每个 run 会永久持有 request/reasoning/text 全文，随使用时长线性增长。 */
const LIVE_RUN_CAP = 20
const LIVE_RUN_TTL_MS = 10 * 60 * 1000
function pruneLiveRuns() {
  const now = Date.now()
  for (const [id, run] of [...liveRuns]) {
    if (run.status === "running") continue
    if (now - (run.endedAt || run.startedAt || now) > LIVE_RUN_TTL_MS) liveRuns.delete(id)
  }
  if (liveRuns.size <= LIVE_RUN_CAP) return
  const finished = [...liveRuns.values()]
    .filter((r) => r.status !== "running")
    .sort((a, b) => (a.endedAt || a.startedAt || 0) - (b.endedAt || b.startedAt || 0))
  for (const run of finished) {
    if (liveRuns.size <= LIVE_RUN_CAP) break
...
async function executeLiveRun(ctx, run) {
  ...
  try {
    const userMsg = relayMessage(run.request, run.direction || null, run.prevText || null, run.sessionContext ? run.sessionContext.block : '')
    if (run.tier === 'extreme') {
      // 极端档：先只读查证项目真实情况（仅用于消歧与校验），再产出命令
      const resolved = resolveSessionCwd(ctx)
      const loop = await runToolLoop(ctx, {
        root: resolved.cwd,
        maxRounds: 3,
        userText: userMsg,
        system: spec.system,
        externalSignal: run.abort.signal,
        onDelta: (type, text) => publishRun(run, { type, text }),
        onReset: () => publishRun(run, { type: 'text-reset' }),
        onTrace: (rows) => publishRun(run, { type: 'trace', rows }),
      })
      if (run.abort.signal.aborted) publishRun(run, { type: 'aborted' })
      else if (loop.error) publishRun(run, { type: 'error', message: loop.error })
      else {
        run.text = String(loop.output || '')
        run.traceCount = loop.trace ? loop.trace.length : 0
        // 循环结束后 capped/converged 才确定：补推一次终态 trace，供浮窗显示「已达轮次上限并收敛」
        publishRun(run, { type: 'trace', rows: loop.trace || [], capped: loop.capped === true, converged: loop.converged === true })
        publishRun(run, { type: 'done', ms: Date.now() - run.startedAt, firstDeltaMs: run.firstDeltaMs, usage: run.usage || null })
      }
    } else {
...
function startLiveRun(ctx, opts) {
  ...
  run.useContext = opts.useContext !== false
  // 会话上下文只在开关打开时读取（隐私：用户可关闭；关闭时完全不碰会话历史）
  run.sessionContext = run.useContext
    ? collectSessionContext(ctx, run.sessionId, { request: run.request })
    : { block: '', turns: 0, messages: 0, reason: 'disabled' }
...
const PKG_NAME = '@lessxi/dsh-prompt-optimizer'
const resolveTrail = []
...
/** v142 · 诊断产物落点：与 STATE_FILE（~/.dsh/prompt-optimizer.json）同一「用户数据目录」，
 *  不再跟随安装位置。动因：ROOT 由 findRoot 解析到 pnpm 安装包目录
 *  （node_modules/.pnpm/<hash>/node_modules/@lessxi/dsh-prompt-optimizer），
 *  pnpm 换一个 hash、或卸载重装，都会让已写下的日志变成孤儿
 *  （实测本机遗留 6 个历史目录、17 个文件、7.65MB），而且用户不可能在
 *  node_modules 深处找到它们，也没法自己清理。 */
const EVIDENCE_HOME = join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'plugins', 'dsh-prompt-optimizer')
const EVIDENCE = join(EVIDENCE_HOME, 'evidence')
/** 旧落点（安装包目录内）。只在启动时做一次性搬迁，之后不再写入。
 *  与 EVIDENCE 指向同一路径时置 null（例如开发态直接跑安装目录的情形）。 */
const LEGACY_EVIDENCE = resolve(join(ROOT, 'evidence')) === resolve(EVIDENCE) ? null : join(ROOT, 'evidence')
...
/** v139 · 有界追加：诊断日志不再无限增长（实测 9 小时 396KB；已卸载的上游版残留 6.7MB）。
 *  用进程内字节计数 + 抽样 stat，避免每次写入都同步 statSync（遥测写盘是高频道路径）。
 *  超过上限时轮转一代：当前文件 → .1（同卷 rename 原子），只保留最近两代。 */
const EVIDENCE_MAX_BYTES = 2 * 1024 * 1024
const EVIDENCE_CHECK_BYTES = 64 * 1024
let evidenceBytesSinceCheck = 0
function appendBounded(file, line) {
  try {
    appendFileSync(file, line)
    evidenceBytesSinceCheck += Buffer.byteLength(line, 'utf8')
    if (evidenceBytesSinceCheck < EVIDENCE_CHECK_BYTES) return
    evidenceBytesSinceCheck = 0
    let size = 0
    try { size = statSync(file).size } catch { return }
    if (size <= EVIDENCE_MAX_BYTES) return
    const prev = file + '.1'
    try { if (existsSync(prev)) unlinkSync(prev) } catch { /* noop */ }
    try { renameSync(file, prev) } catch { /* noop */ }
  } catch { /* evidence is best-effort */ }
}
/** djb2 指纹（与 collectSessionContext 内联实现同算法）：不落原文也能比对内容是否变化。 */
function fingerprint(text) {
  let fp = 5381
  const s = String(text || '')
  for (let k = 0; k < s.length; k += 1) fp = ((fp << 5) + fp + s.charCodeAt(k)) >>> 0
  return fp.toString(16)
}
/** v139 · 调试取证开关（默认关）：显式开启后才把截断文本写进诊断日志。
 *  调用频率 = 每条会话消息（每分钟几条），读一个约 2KB 状态文件成本可忽略，故不做缓存（避免缓存失效漏洞）。 */
function debugEvidenceOn() {
  try { return loadPluginState().debugEvidence === true } catch { return false }
}
function ensureDir() {
  try { mkdirSync(EVIDENCE, { recursive: true }) } catch { /* ignore */ }
}
/** v142 · 把旧落点里的诊断产物搬到用户数据目录。保守三原则：
 *  ① 只动直接子文件（与 /evidence/clear 同一策略，不递归、不碰目录）；
 *  ② 目标已存在则保留旧件、绝不覆盖（宁可留孤儿也不丢数据）；
 *  ③ rename 失败（跨盘 EXDEV）降级为 copy + unlink，仍失败则只记 failed。
 *  幂等：搬空后旧目录为空，之后每次启动都是空转。 */
function migrateLegacyEvidence() {
  const out = { from: LEGACY_EVIDENCE, to: EVIDENCE, moved: [], kept: [], failed: [] }
  if (!LEGACY_EVIDENCE) return null
  try {
    if (!existsSync(LEGACY_EVIDENCE)) return null
    ensureDir()
    for (const name of readdirSync(LEGACY_EVIDENCE)) {
      const srcPath = join(LEGACY_EVIDENCE, name)
      try { if (!statSync(srcPath).isFile()) continue } catch { continue }
      const dstPath = join(EVIDENCE, name)
      if (existsSync(dstPath)) { out.kept.push(name); continue }
      try {
        renameSync(srcPath, dstPath)
      } catch {
        try { copyFileSync(srcPath, dstPath); unlinkSync(srcPath) } catch { out.failed.push(name); continue }
      }
      out.moved.push(name)
    }
...
/** 启动即迁移一次（模块求值到这里时 const 均已就绪）。结果经 /state 的 diag 暴露。 */
let legacyMigration = null
try { legacyMigration = migrateLegacyEvidence() } catch { /* evidence is best-effort */ }

/** 读盘失败留痕（区分「文件不存在」与「文件损坏」），供 /state 暴露。 */
const readErrors = new Map()
function readJson(file, fallback) {
  try { const parsed = JSON.parse(readFileSync(file, 'utf8').replace(/^\uFEFF/, '')); readErrors.delete(file); return parsed } catch (error) {
    try { if (existsSync(file)) readErrors.set(file, String((error && error.message) || error)) } catch { /* noop */ }
    return fallback
  }
}
...
export function apply(ctx) {
  ensureDir()
  // settings 是可选依赖：走 ctx.inject 才能在服务就绪时拿到它（官方同款姿势）
  try { ctx.inject(['settings'], (settingsCtx) => { initSettingsNamespace(settingsCtx) }) } catch { initSettingsNamespace(ctx) }
  const events = []
  let cmdHits = 0
  ...
  ctx.effect(() => {
    const stop = ctx.setInterval(() => {
      ...
      for (const [match, expiresAt] of [...sweepWatches]) {
        ...
        if (removed.length > 0) {
          try { appendBounded(EVENTS_FILE, JSON.stringify({ t: now, watchdog: match, removed }) + '\n') } catch { /* best effort */ }
        }
      }
    ...
    const off = ctx.on('session/event', (session, event) => {
      try {
        ...
        const rec = {
          ...
          fp: fingerprint(text),
          injected: isInjectedUserText(text),
          rpcId: data.source && data.source.rpcId ? String(data.source.rpcId) : null,
        }
        // v139：默认只留「规模 + 指纹 + 是否系统注入」，不落原文（会话内容比单条输入敏感）
        if (debugEvidenceOn()) rec.text = String(text).slice(0, 240)
        events.push(rec)
        if (events.length > 400) events.shift()
        appendBounded(EVENTS_FILE, JSON.stringify(rec) + '\n')
      ...
      try {
        if (url.includes('/cmd')) {
          ...
          if (expired && cmd && cmd.run) {
            ...
            try { appendBounded(EVENTS_FILE, JSON.stringify({ t: Date.now(), cmdExpired: cmd.token || cmd.run, ageMs }) + '\n') } catch { /* best effort */ }
            cmd = null
          }
          return send(200, {
            ok: true,
            host: '@lessxi/dsh-prompt-optimizer',
            ts: Date.now(),
            cmd,
            cmdAgeMs: ageMs,
            cmdTtlMs: CMD_TTL_MS,
            ...
            cmdHits,
            diag: { startUrl: START_URL, trail: resolveTrail, root: ROOT, evidence: EVIDENCE, legacyEvidence: LEGACY_EVIDENCE, legacyMigration, cmdFile: CMD_FILE, raw: raw === null ? null : raw.trim(), readError, exists: existsSync(CMD_FILE) },
          })
        }
        ...
        if (url.includes('/state')) {
          ...
          return send(200, { ok: true, state: loadPluginState(), file: STATE_FILE, settings: settingsStatus, readError: readErrors.get(STATE_FILE) || null })
        }
        /* v140 · 用户隐私控制面：清空本机诊断日志。
           只删 EVIDENCE 目录下的直接子文件（全是本插件自己写的诊断产物），不递归、不碰目录。
           证据文件是 best-effort：单个文件失败不阻断其余，目录不存在视为「本来就没有」。 */
        if (url.includes('/evidence/clear')) {
          ...
          evidenceBytesSinceCheck = 0
          return send(200, { ok: true, removed, bytesFreed, dir: EVIDENCE })
}
        ...
        if (url.includes('/run')) {
          ...
          let payload = {}
          try { payload = JSON.parse(body || '{}') } catch { payload = {} }
          // 纵深防御（v138）：客户端已前置拦截，这里兜住其他调用方（探针/自检/直接打 API）
          const reqText = String((payload && payload.request) || '')
          if (reqText.length > MAX_REQUEST_CHARS) {
            return send(400, { ok: false, error: 'request-too-long', chars: reqText.length, max: MAX_REQUEST_CHARS })
          }
          const run = startLiveRun(ctx, payload)
          return send(200, { ok: true, runId: run.id, tier: run.tier, forceError: run.forceError, useContext: run.useContext !== false, contextTurns: run.sessionContext ? run.sessionContext.turns || 0 : 0, contextReason: run.sessionContext ? run.sessionContext.reason || null : null, contextChars: run.sessionContext ? run.sessionContext.chars || 0 : 0 })
        }
        ...
        if (url.includes('/beacon')) {
          const body = await readBody(req)
          ensureDir()
          appendBounded(join(EVIDENCE, 'client-beacon.jsonl'), String(body) + '\n')
        ...
        if (url.includes('/report')) {
          ...
          appendBounded(join(EVIDENCE, 'selftest-reports.jsonl'), JSON.stringify(report) + '\n')
