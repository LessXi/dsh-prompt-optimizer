...
function mirrorToSettings(patch) {
  try {
    ...
    const mirror = { tier: patch.tier, permission: patch.permission, model: patch.model === undefined ? null : patch.model }
    const key = JSON.stringify([mirror.tier, mirror.permission, mirror.model])
