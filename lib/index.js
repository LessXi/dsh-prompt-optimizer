...
function initSettingsNamespace(settingsCtx) {
  try {
    ...
    const schema = z.object({
      ...
      model: z.object({ provider: z.string(), model: z.string(), name: z.string().required(false) }).required(false),
      ui: z.object({ w: z.number().required(false), h: z.number().required(false), x: z.number().required(false), y: z.number().required(false) }).required(false),
      useContext: z.boolean().default(true),
      debugEvidence: z.boolean().default(false),
    })
      ...
      if (settingsScope && typeof settingsScope.watch === 'function') {
        settingsScope.watch((next) => {
          try {
            ...
            if (JSON.stringify([next.tier, next.permission, next.useContext, next.debugEvidence]) === lastMirrorFingerprint) return
            const cur = loadPluginState()
            const patch = {}
            if (typeof next.tier === 'string' && next.tier !== cur.tier) patch.tier = next.tier
            if (typeof next.permission === 'string' && next.permission !== cur.permission) patch.permission = next.permission
            if (typeof next.useContext === 'boolean' && next.useContext !== (cur.useContext !== false)) patch.useContext = next.useContext
            // v153：设置面板里的「诊断取证」原本无人转发 ⇒ 开关完全无效（值永远停在 false）
            if (typeof next.debugEvidence === 'boolean' && next.debugEvidence !== (cur.debugEvidence === true)) patch.debugEvidence = next.debugEvidence
...
function mirrorToSettings(patch) {
  try {
    ...
    // 只镜像真正的用户配置：浮窗几何（ui）是插件内部状态，不进 DSH 设置文档。
    // 值未变则直接返回 —— 否则每次拖动浮窗 / 每次落盘都会 bump 设置 revision 并广播事件（同 v139 的 labels 去重）。
    // v153：镜像必须覆盖设置 schema 暴露的**每一个**字段。此前只镜像 tier/permission/model，
    // 于是设置文档里的 useContext/debugEvidence 永远停在 schema 默认值（true/false）；
    // 而 settingsScope.watch 的反向同步会把那个陈旧默认值当权威写回本插件 —— 实测后果：
    // 用户关闭「会话背景」后只要再换一次档位，开关就被静默改回开启（隐私选择被撤销）。
    const mirror = {
      tier: patch.tier,
      permission: patch.permission,
      model: patch.model === undefined ? null : patch.model,
      useContext: patch.useContext === undefined ? true : patch.useContext === true,
      debugEvidence: patch.debugEvidence === true,
    }
    const key = JSON.stringify([mirror.tier, mirror.permission, mirror.model, mirror.useContext, mirror.debugEvidence])
    if (key === lastMirrorKey) return
    lastMirrorKey = key
    lastMirrorFingerprint = JSON.stringify([mirror.tier, mirror.permission, mirror.useContext, mirror.debugEvidence])
    Promise.resolve(settingsScope.update(mirror)).then(
      () => { settingsStatus = 'registered+mirrored' },
      (error) => { settingsStatus = 'registered (mirror-error: ' + String(error) + ')' },
    )
  } catch (error) {
...
function savePluginState(patch) {
  ...
  mirrorToSettings({ tier: next.tier, permission: next.permission, model: next.model, ui: next.ui, useContext: next.useContext, debugEvidence: next.debugEvidence })
  return next
}
