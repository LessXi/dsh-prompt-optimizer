 ...
 * 不含任何优化逻辑（属后续小类，禁入区）。
 */
import { mkdirSync, readFileSync, writeFileSync, appendFileSync, existsSync, statSync, readdirSync, realpathSync, renameSync } from 'node:fs'
...
function collectSessionContext(ctx, sessionId, opts) {
  ...
  try {
    ...
    try {
      ...
      appendFileSync(join(EVIDENCE, 'session-context.jsonl'), JSON.stringify({
